...
namespace ZenPlatform.Tests.SqlBuilder.Postgres
{
    public class SelectTest
    {
            ...
            var query = new SelectQueryNode().From("conf").Select("Data")
                .Where(f =>
                    f.And(_ =>
                        new ConditionNode[]
                        {
                            f.Condition(fel => fel.Field("BlobName"), Tokens.Comparators.E,
                                fer => fer.Parameter("BlobName")),
        ...
        public void RenameTest()
        {
            ...
            Assert.Equal("ALTER TABLE \"some_schema\".\"test\" RENAME TO \"new_test\"", c.Compile(query));
        }
    }
}