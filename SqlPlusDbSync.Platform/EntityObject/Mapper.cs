...
namespace SqlPlusDbSync.Platform.EntityObject
{
    public class Mapper
    {
        public DTOObject Map(DTOObject dtoObject, SType sobject, SqlDataReader reader)
        {
            ...
            if (!reader.IsClosed && reader.HasRows)
            {
                while (reader.Read())
                {
                    ...
                    MapObject(dtoObject, sobject, reader, path, false);
                }
            }
            else
            {
                dtoObject.IsDeleted = true;
            }

            return dtoObject;
        }

        public DTOObject MapGraph(SType sobject, SqlDataReader reader, Type type)
        {
            var entity = Activator.CreateInstance(type) as DTOObject;
        ...
        private void MapRelations(object entity, SType sobject, SqlDataReader reader, string[] path, bool graphOnly)
        {
            ...
            foreach (var rel in sobject.Relations)
            {
                ...
                var newInstance = Activator.CreateInstance(relObjectType) as DTOObject;
                ...
                if (index >= 0)
                    newInstance = collection[index] as DTOObject;
                else
        ...
        {
            DTOObject ent = entity as DTOObject;
