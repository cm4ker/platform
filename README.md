...
После этого остро встал вопрос о том, что же использовать для отображения данных пользователю. Это также должен быть кросплатформенный UI. Для этого прекрасно подходит фрейморк для кросплатформенной разработки интерфейсов Avalonia. Мало того что это проект со свободной лицензией, так ещё построен на принципах ```WPF``` (также используется язык разметки ```XAML```). 
 (Обновление 07.12.2018) 4.12.2018 MS выложил wpf в OpenSource в связи с этим станет вопрос времени портирования этой технологии на остальные операционные системы. Для него написано так много компонентов, так что я думаю что есть смысл подаждать и не делать весь тулинг на Avalonia, а пока что довольствоваться wpf разработкой.
...
10. Подсистема обработки сообщений

11. Подсистема кэширвоания( redis )
...
Для того, чтобы отобразить данные на клиенте, их нужно передать. Допустим у нас есть объект следующей структуры:
...
И нам необходимо на форме сделать отображение следующих реквизитов:
     
     Полностью документ "Накладная"
     Поставщик.Страна.Наименование 
     Поставщик.Адрес 
     Склад.Подразделение.Наименование

Для этого нам нужно передать эти элементы на клиента (и только их).
Для этого предназначено **"Клиентское локальное хранилище объектов"** (InMemory Client Object Storage)

**<span style="color: red">Редактируемый объект должен быть передан полностью!!</span>**


С тем что у нас есть локальное хранилище мы определились. Далее необходимо понять, что же нам нужно передавать.
Для этого мы берем запрос, который генерируется платформой.

    Select * From Документы.Накладная Where Id = @Id (Уровень 0) 
    Select * From Справочники.Поставщики Where Id = @Id (Уровень 1)
    Select * From Справоники.Страна Where Id = @Id
...
Мы итерируем их все и просто кидаем на клиента, один за другим.

Это называется "запрос дерева объектов"(tree object lookup)
