...
Сборка, которая отдается клиенту должна содержать в себе определённый набор команд
...
{
    // Родительская комманда
    Command Parent { get; set; }
    ...
    // Куда приккрепляется комманда (Главное меню, Объект, Боковое меню... и т.д.)
