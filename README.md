...
Было принято решение об унификации работы с интерфейсом.

Пометка: Интерфейс должен компилироваться перед компилированием комманд на клиенте.

Комопненты используют только стандартный набор API платформы. Появится единообразие в виде программы на выходе.

Для разработки интерфейса необходимо проделать следующие шаги:


1) Создать Окружение для сервера
2) Передать данные с сервера на клиент
3) Обработать эти данные
4) Передать данные с клиента на сервер

У нас есть следующие предпосылки:

1) Язык должен быть простым и удобным в чтении и понимании, что-то вроде
...
3) Можно создать фрагмент, присоединить к нему код. Фрагменты можно создавать только на сервере и передавать их на клиента.
...
<module>
    ...
    void OnFieldChange()
    {
        ...
        {
            field.Text = field.Text.Slise(1,8);
        }
    }
