...
Для этого необходимо описать структуру и возможности. Пример формы и её рендеринг:

```xml
<!-- Поле "ТипДанных"(data_type) Представлены тут схематично. Это, конечно же, перечисление -->
<form data_type="Document.ПриходнаяНакладная">
    
    <!-- Для фрагмента важно указать тип -->
    <fragment id="1" data_type="Document.ПриходнаяНакладная">
