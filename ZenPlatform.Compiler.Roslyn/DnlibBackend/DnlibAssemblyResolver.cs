...
{
    public class SreAssemblyResolver : IAssemblyResolver
    {
...
//            }
            ...
            if (!libraries.TryGetValue(libname, out def))
            {
                foreach (var path in paths)
                {
                    ...
                    if (File.Exists(dllPath))
                    {
                        def = AssemblyDef.Load(dllPath,
                            new ModuleContext(this, new SreMetadataResolver(this)));
                ...
                if (asm != null)
                {
                    asm.ManifestModule.Context = new ModuleContext(this, new SreMetadataResolver(this));
                    return asm;
                }
