...
class_definition
	: (attributes? all_member_modifiers?) CLASS identifier type_parameter_list? class_base? type_parameter_constraints_clauses?
	    class_body ';'?
	;

struct_definition
	: (attributes? all_member_modifiers?) STRUCT identifier type_parameter_list? struct_interfaces? type_parameter_constraints_clauses?
	    struct_body ';'?
	;

interface_definition
	: (attributes? all_member_modifiers?) INTERFACE identifier variant_type_parameter_list? interface_base?
	    type_parameter_constraints_clauses? interface_body ';'?
	;

enum_definition
	: (attributes? all_member_modifiers?) ENUM identifier enum_base? enum_body ';'?
	;

delegate_definition
	: (attributes? all_member_modifiers?) DELEGATE return_type identifier variant_type_parameter_list?
	  OPEN_PARENS formal_parameter_list? CLOSE_PARENS type_parameter_constraints_clauses? ';'
	;

event_declaration
	: (attributes? all_member_modifiers?) EVENT type (variable_declarators ';' | member_name OPEN_BRACE event_accessor_declarations CLOSE_BRACE)
	;
...
property_declaration // Property initializer & lambda in properties C# 6
	: type member_name (OPEN_BRACE accessor_declarations CLOSE_BRACE ('=' variable_initializer ';')? | right_arrow expression ';')
	;
...
method_declaration // lamdas from C# 6
	: (type | VOID) method_member_name type_parameter_list? OPEN_PARENS formal_parameter_list? CLOSE_PARENS
