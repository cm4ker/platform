...
namespace BoundTreeGenerator
{
    ...
    internal sealed class BoundNodeClassWriter
    {
        ...
        private NullHandling FieldNullHandling(TreeType node, string fieldName)
        {
            ...
            if (f.Null != null)
            {
                if (_targetLang == TargetLanguage.Aquila && (f.Null.ToUpperInvariant() is ("ALLOW" or "ALWAYS")) && !f.Type.EndsWith('?') && !IsValueType(f.Type))
                {
        ...
        private void WriteType(TreeType node)
        {
            ...
            foreach (var field in (_targetLang == TargetLanguage.Aquila ? FieldsIncludingOverrides(node) : Fields(node)))
            {
        ...
        private bool IsImmutableArray(string typeName, out string elementType)
        {
            ...
            {
                TargetLanguage.Aquila => "ImmutableArray<",
