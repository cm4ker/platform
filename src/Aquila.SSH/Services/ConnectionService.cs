...
using Aquila.SSH.Messages;
using Aquila.SSH.Messages.Connection;

namespace Aquila.SSH.Services
{
    public class ConnectionService : SshService, IDynamicInvoker
    {
        ...
        private void HandleMessage(ChannelRequestMessage message)
        {
            switch (message.RequestType)
            {
                ...
                case "subsystem":
                    ...
                    break;
                case "window-change":
                    var chn_msg = Message.LoadFrom<ChannelWindowChangeMessage>(message);
                    HandleMessage(chn_msg);
                    break;
        ...
        private void HandleMessage(ChannelWindowChangeMessage message)
        {
            var channel = FindChannelByServerId<Channel>(message.RecipientChannel);
            channel.OnSizeChanged(message.Size);
        }
