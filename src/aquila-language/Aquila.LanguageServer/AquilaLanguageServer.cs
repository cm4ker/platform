...
namespace Aquila.LanguageServer
{
    internal class AquilaLanguageServer : ILogTarget
    {
        ...
        static Version LatestAquilaVersion
        {
            get
            {
                var v4 = typeof(AquilaCompilation).Assembly.GetName().Version;
  ...
  Aquila Version: {(sdkverstr ?? "unknown")}");
        ...
        private void ProcessGoToDefinition(object requestId, TextDocumentPositionParams docPosition)
        {
            ...
            if (_project != null)
            {
                ...
                try
                {
                    var locations = _project.ObtainDefinition(filepath, docPosition.Position.Line,
                        docPosition.Position.Character);
