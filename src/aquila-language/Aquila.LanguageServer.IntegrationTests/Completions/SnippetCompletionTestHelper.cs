...
namespace Aquila.LanguageServer.IntegrationTests.Completions
{
    ...
    {
        private static readonly Regex PlaceholderCommentPattern = new Regex(@"\/\/\s*\$(?<index>\d+)\s*=\s*(?<text>.*)",
        ...
        //     }
        //
        //     return snippet.Format((s, placeholder) =>
        //         placeholder.Index >= 0 && placeholder.Index <= replacementsMap.Count
        //             ? (replacementsMap.ContainsKey(placeholder.Index)
        //                 ? replacementsMap[placeholder.Index]
        //                 : string.Empty)
        //             : string.Empty);
        // }
    }
