...
namespace Aquila.LanguageServer.IntegrationTests.Assertions
{
    ...
    public class RangeAssertions : ReferenceTypeAssertions<Range, RangeAssertions>
    {
        ...
        public AndConstraint<RangeAssertions> HaveRange((int line, int character) start, (int line, int character) end,
            string because = "", params object[] becauseArgs)
        {
            Execute.Assertion
                ...
                .FailWith("Expected start to be at [{0}:{1}] but found [{2}:{3}]", _ => start.line,
                    _ => start.character, p => p.Line, p => p.Character)
                .Then
                .Given<Position>(_ => Subject.End)
                .ForCondition(p => p.Line == end.line && p.Character == end.character)
                .FailWith("Expected end to be at [{0}:{1}] but found [{2}:{3}]", _ => end.line, _ => end.character,
                    p => p.Line, p => p.Character);
