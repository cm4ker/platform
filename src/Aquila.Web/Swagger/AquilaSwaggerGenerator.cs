...
internal class AquilaSwaggerGenerator : ISwaggerProvider
{
    ...
    private readonly AquilaApiHolder _apiHolder;

    private readonly SwaggerGeneratorOptions _options;

    public AquilaSwaggerGenerator(SwaggerGeneratorOptions options,
        IApiDescriptionGroupCollectionProvider apiDescriptionsProvider,
        ISchemaGenerator schemaGenerator,
        AquilaApiHolder apiHolder,
        AquilaHandlerMiddleware middleware)
    {
        _options = options ?? new SwaggerGeneratorOptions();
        _apiDescriptionsProvider = apiDescriptionsProvider;
        _schemaGenerator = schemaGenerator;
        _apiHolder = apiHolder;
    }
    ...
    public OpenApiDocument GetSwagger(string documentName, string host = null, string basePath = null)
    {
        ...
        var applicableApiDescriptions = _apiDescriptionsProvider.ApiDescriptionGroups.Items
            ...
            .Where(apiDesc =>
                !(_options.IgnoreObsoleteActions && apiDesc.CustomAttributes().OfType<ObsoleteAttribute>().Any()))
        ...
        swaggerDoc.Components.Schemas =
            new SortedDictionary<string, OpenApiSchema>(swaggerDoc.Components.Schemas, _options.SchemaComparer);
    ...
    private OpenApiPaths GeneratePaths(IEnumerable<ApiDescription> apiDescriptions, SchemaRepository schemaRepository)
    {
        ...
        List<(string, OpenApiPathItem)> list = GeneratePathItemFromApiHolder(schemaRepository);

        foreach (var item in list)
        {
            paths.Add(item.Item1, item.Item2);
        }

        return paths;
    }

    private List<(string, OpenApiPathItem)> GeneratePathItemFromApiHolder(SchemaRepository schemaRepository)
    {
        var result = new List<(string, OpenApiPathItem)>();

        foreach (var item in _apiHolder.Endpoints)
        {
            //const path to the api is /api/{instance}/endpoint/{method_name}
            var pathName = $"/api/{item.Key.instanceName}/endpoint/{item.Key.methodName}";

            result.Add(
                (pathName, new OpenApiPathItem
                {
                    Description = "The method from the test",
                    Operations = { { OperationType.Get, GenerateOperationFromMethod(item.Value, schemaRepository) } },
                }));
        }

        foreach (var crud in _apiHolder.Cruds)
        {
            //const path to the api is /api/{instance}/{object_name}/{method_name}
            var pathName = $"/api/{crud.Key.instanceName}/{crud.Key.objectName}/{crud.Key.methodName}";

            result.Add(
                (pathName, new OpenApiPathItem
                {
                    Description = "The method from the test",
                    Operations = { { OperationTypeMap[crud.Key.operationType], GenerateOperationFromMethod(crud.Value, schemaRepository) } },
                }));
        }
    ...
    private OpenApiOperation GenerateOperationFromMethod(MethodInfo itemValue, SchemaRepository schemaRepository)
    {
        var parameters = itemValue.GetParameters();
            ...
            {
                {
                    ...
                    {
                        Schema = GenerateSchemaFromFormParameters(parameters, schemaRepository)
                    }
                }
            }
        };
    ...
    private OpenApiOperation GenerateOperation(ApiDescription apiDescription, SchemaRepository schemaRepository)
    {
        ...
        catch (Exception ex)
        {
            throw new SwaggerGeneratorException(
                message:
                $"Failed to generate Operation for action - {apiDescription.ActionDescriptor.DisplayName}. See inner exception",
                innerException: ex);
        }
    }
    ...
    private IList<OpenApiParameter> GenerateParameters(ApiDescription apiDescription,
    ...
    private OpenApiSchema GenerateSchema(Type type, SchemaRepository schemaRepository, PropertyInfo propertyInfo = null,
        ParameterInfo parameterInfo = null, ApiParameterRouteInfo routeInfo = null)
    {
        try
        {
    ...
    private OpenApiSchema GenerateSchemaFromFormParameters(
        IEnumerable<ParameterInfo> formParameters,
        SchemaRepository schemaRepository)
    {
    ...
    private IEnumerable<string> InferResponseContentTypes(ApiDescription apiDescription,
        ApiResponseType apiResponseType)
    {
    ...
    private static readonly Dictionary<BindingSource, ParameterLocation> ParameterLocationMap =
        new Dictionary<BindingSource, ParameterLocation>
        {
