...
namespace Pchp.CodeAnalysis.Semantics
{
    ...
    internal class SemanticsBinder
    {
            ...
            SourceTypeSymbol self = null)
        {
            ...
            routine.Syntax.Properties.TryGetProperty(
                out ImmutableArray<AST.IYieldLikeEx> yields); // routine binder sets this property
        ...
        BoundStatement BindStatementCore(Statement stmt)
        {
            ...
            // if (stmt is JumpStmt jmpStm) return BindJumpStmt(jmpStm);
        ...
        protected BoundExpression BindExpressionCore(Expression expr, BoundAccess access)
        {
            ...
            if (expr is Literal) return BindLiteral((AST.Literal) expr).WithAccess(access);
            if (expr is ConstantUse) return BindConstUse((AST.ConstantUse) expr).WithAccess(access);
            ...
            if (expr is UnaryEx) return BindUnaryEx((AST.UnaryEx) expr, access).WithAccess(access);
            if (expr is IncDecEx) return BindIncDec((AST.IncDecEx) expr).WithAccess(access);
            if (expr is ConditionalEx)
                return BindConditionalEx((ConditionalEx) expr, access).WithAccess(access);
            if (expr is ConcatEx) return BindConcatEx((AST.ConcatEx) expr).WithAccess(access);
            if (expr is IncludingEx) return BindIncludeEx((AST.IncludingEx) expr).WithAccess(access);
            if (expr is InstanceOfEx) return BindInstanceOfEx((AST.InstanceOfEx) expr).WithAccess(access);
            if (expr is PseudoConstUse) return BindPseudoConst((AST.PseudoConstUse) expr).WithAccess(access);
            if (expr is IssetEx) return BindIsSet((AST.IssetEx) expr).WithAccess(access);
            if (expr is ExitEx) return BindExitEx((AST.ExitEx) expr).WithAccess(access);
            if (expr is ThrowEx throwEx)
                return new BoundThrowExpression(BindExpression(throwEx.Expression, BoundAccess.Read));
            if (expr is EmptyEx) return BindIsEmptyEx((AST.EmptyEx) expr).WithAccess(access);
            if (expr is LambdaFunctionExpr) return BindLambda((AST.LambdaFunctionExpr) expr).WithAccess(access);
            if (expr is EvalEx) return BindEval((AST.EvalEx) expr).WithAccess(access);
            if (expr is YieldEx) return BindYieldEx((AST.YieldEx) expr, access).WithAccess(access);
            if (expr is YieldFromEx) return BindYieldFromEx((AST.YieldFromEx) expr, access).WithAccess(access);
            if (expr is ShellEx) return BindShellEx((AST.ShellEx) expr).WithAccess(access);
        ...
        protected BoundLambda BindLambda(LambdaFunctionExpr expr)
        {
            ...
            if (expr is AST.ArrowFunctionExpr fn)
            {
                ...
                useparams = captured
                    .Select(vname => new AST.FormalParam(Span.Invalid, vname.Value, Span.Invalid, null,
                        AST.FormalParam.Flags.Default, null))
                    .ToList();
            }
            else
            {
        ...
        BoundExpression BindIsMemberOfChain(AST.Expression expr, BoundAccess access)
        {
            ...
            if (expr is AST.FunctionCall varlike && varlike.IsMemberOf != null
            ...
            else if (x is AST.StaticMtdCall staticMtdCall)
            {
                ...
                var boundname = (staticMtdCall is AST.DirectStMtdCall dm)
                    ...
                    : new BoundRoutineName(new BoundConversionEx(
                        BindExpression(((AST.IndirectStMtdCall) staticMtdCall).MethodNameExpression),
        ...
        public IBoundTypeRef BindTypeRef(AST.TypeRef tref, bool objectTypeInfoSemantic = false)
        {
            ...
            // update flags
            if (type is BoundReservedTypeRef rt && rt.ReservedType == AST.ReservedTypeRef.ReservedType.@static &&
                Routine != null)
            {
        ...
        protected BoundExpression BindSimpleVarUse(AST.SimpleVarUse expr, BoundAccess access)
        {
                ...
                if (varname.IsDirect)
                {
                    if (varname.NameValue.IsThisVariableName)
                    {
                        ...
                        if (Routine != null && Routine.IsStatic && !Routine.IsGlobalScope &&
                            !(Routine is SourceLambdaSymbol lambda && lambda.UseThis))
                        {
                            // WARN:
                            Diagnostics.Add(DiagnosticBagExtensions.ParserDiagnostic(ContainingFile, expr.Span,
                                Devsense.PHP.Errors.Warnings.ThisOutOfMethod));
            ...
            else
            {
                ...
                return BoundFieldRef.CreateInstanceField(BindExpression(expr.IsMemberOf, instanceAccess), varname)
                    .WithAccess(access);
            }
        }
    ...
    internal sealed class GeneratorSemanticsBinder : SemanticsBinder
    {
        ...
        protected override BoundExpression BindBinaryEx(AST.BinaryEx expr)
        {
            ...
            if (!(expr.Operation == AST.Operations.And || expr.Operation == AST.Operations.Or ||
                  expr.Operation == AST.Operations.Coalesce))
            {
        ...
        private BoundItemsBag<BoundExpression> BindExpressionWithSeparatePreBoundStatements(AST.Expression expr,
            BoundAccess access)
        {
            if (expr == null)
            {
