...
namespace Aquila.Syntax.Syntax
{
    ...
    {
        public static Text.Span SafeSpan(int start, int length) =>
            start < 0 || length < 0 ? Text.Span.Invalid : new Text.Span(start, length);
        ...
        public static Text.Span SpanIntermission(Text.Span previous, Text.Span next) =>
            previous.IsValid && next.IsValid && previous.End <= next.Start
                ? Text.Span.FromBounds(previous.End, next.Start)
                : Text.Span.Invalid;

        public static Text.Span SpanIntermission(int previous, Text.Span next) =>
            previous >= 0 && next.IsValid && previous <= next.Start
                ? Text.Span.FromBounds(previous, next.Start)
                : Text.Span.Invalid;

        public static Text.Span SpanIntermission(Text.Span previous, int next) =>
            previous.IsValid && next >= 0 && previous.End <= next
                ? Text.Span.FromBounds(previous.End, next)
                : Text.Span.Invalid;
    ...
    {
        /// <summary>
        /// Increments a string in a manner of Perl. 
        /// </summary>
        ...
        public static string Increment(string str)
        {
            ...
            {
                case '9':
                    result.Insert(0, new char[] { '1' });
                    break;
                case 'z':
                    result.Insert(0, new char[] { 'a' });
                    break;
                case 'Z':
                    result.Insert(0, new char[] { 'A' });
                    break;
            ...
            for (int i = 0; i < str.Length; i++)
            {
                ...
                switch (c)
                {
                    case '\\':
                        ...
                        break;
                    case '\0':
                        result.Append(slashed_nul);
                        break;
                    case '\'':
                        result.Append(single_quotes);
                        break;
                    case '"':
        ...
        /// </summary>
        public static int CharsCount(this string str, char c, int maxCount)
        {
            if (str == null)
                return 0;
    ...
    public static class ArrayUtils
    {
        ...
        /// Empty int array.
        /// </summary>
        public static int[] EmptyIntegers
        {
            get { return EmptyArray<int>.Instance; }
        }

        /// <summary>
        /// Empty ushort array.
        /// </summary>
        public static ushort[] EmptyUShorts
        {
            get { return EmptyArray<ushort>.Instance; }
        }
        ...
        {
            get { return EmptyArray<byte>.Instance; }
        }
