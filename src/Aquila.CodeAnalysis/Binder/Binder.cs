...
using AquilaCompilation = Aquila.CodeAnalysis.AquilaCompilation;
...
namespace Microsoft.CodeAnalysis.CSharp
{
    ...
    {
        internal AquilaCompilation Compilation { get; }
        ...
        protected Binder(Binder next, BinderFlags flags)
        {
            ...
            // Implied.
            Debug.Assert(!flags.Includes(BinderFlags.InNestedFinallyBlock) ||
        ...
        /// </summary>
        internal virtual ImmutableArray<SourceLocalSymbol> GetDeclaredLocalsForScope(LangElement scopeDesignator)
        {
        ...
        {
            get { return Next.ContainingMemberOrLambda; }
        }
        ...
        {
            get { return Next.IsInMethodBody; }
        }
        ...
        {
            get { return Next.IsDirectlyInIterator; }
        }
        ...
        {
            get { return Next.IsIndirectlyInIterator; }
        }
        ...
        // {
        //     get { return _next.QuickAttributeChecker; }
        // }
        ...
        {
            get { return this.Next.ConstantFieldsInProgress; }
        }

        internal virtual ConsList<FieldSymbol> FieldsBeingBound
        {
            get { return this.Next.FieldsBeingBound; }
        }

        internal virtual SourceLocalSymbol LocalInProgress
        {
            get { return this.Next.LocalInProgress; }
        }

        internal virtual BoundExpression ConditionalReceiverExpression
        {
            get { return this.Next.ConditionalReceiverExpression; }
        }
        ...
        internal static void Error(DiagnosticBag diagnostics, ErrorCode code, LangElement syntax)
        {
        ...
        /// </summary>
        internal void ReportDiagnosticsIfObsolete(DiagnosticBag diagnostics, Symbol symbol, SyntaxNodeOrToken node,
            bool hasBaseReceiver)
        {
            ...
            //         ReportDiagnosticsIfObsolete(diagnostics, symbol, node, hasBaseReceiver,
            //             this.ContainingMemberOrLambda, this.ContainingType, this.Flags);
            //         break;
            // }
        }

        internal void ReportDiagnosticsIfObsolete(DiagnosticBag diagnostics, Conversion conversion,
            SyntaxNodeOrToken node, bool hasBaseReceiver)
        {
            ...
            BinderFlags location)
        {
            ...
            // }
            //
            // var diagnosticKind = ReportDiagnosticsIfObsoleteInternal(diagnostics, leastOverriddenSymbol, node,
            //     containingMember, location);
            //
            ...
            //         if (checkOverridingSymbol)
            //         {
            //             Debug.Assert(diagnosticKind != ObsoleteDiagnosticKind.Lazy,
        ...
        internal static ObsoleteDiagnosticKind ReportDiagnosticsIfObsoleteInternal(DiagnosticBag diagnostics,
            Symbol symbol, SyntaxNodeOrToken node, Symbol containingMember, BinderFlags location)
        {
            ...
            TypeSymbol throughTypeOpt = null)
        {
            return this.Flags.Includes(BinderFlags.IgnoreAccessibility) ||
                   AccessCheck.IsSymbolAccessible(symbol, within, ref useSiteDiagnostics, throughTypeOpt);
        }
        ...
        internal BoundExpression WrapWithVariablesIfAny(LangElement scopeDesignator, BoundExpression expression)
        {
            // var locals = this.GetDeclaredLocalsForScope(scopeDesignator);
            // return (locals.IsEmpty)
            //     ? expression
            //     : new BoundSequence(scopeDesignator, locals, ImmutableArray<BoundExpression>.Empty, expression,
            //         expression.Type) {WasCompilerGenerated = true};
            throw new NotImplementedException();
        }

        internal BoundStatement WrapWithVariablesIfAny(LangElement scopeDesignator, BoundStatement statement)
        {
        ...
        /// </summary>
        internal BoundStatement WrapWithVariablesAndLocalFunctionsIfAny(CSharpSyntaxNode scopeDesignator,
            ...
            (string description, string snippet, string locals) Print(Binder scope)
            {
                ...
                if (scope.ScopeDesignator != null)
                {
                    ...
                    else
                    {
                        ...
                        snippet = first.Substring(0, Math.Min(first.Length, 12)) + " ... " +
