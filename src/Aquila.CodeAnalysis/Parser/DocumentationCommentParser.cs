    ...
    internal class DocumentationCommentParser : SyntaxParser
    {
        ...
        public DocumentationCommentTriviaSyntax ParseDocumentationComment(out bool isTerminated)
        {
            ...
            try
            {
                ...
                isTerminated = !_isDelimited || (eoc.LeadingTrivia.Count > 0 &&
        ...
        public void ParseRemainder(SyntaxListBuilder<XmlNodeSyntax> nodes)
        {
            ...
            try
            {
                ...
                XmlParseErrorCode code =
                    endTag ? XmlParseErrorCode.XML_EndTagNotExpected : XmlParseErrorCode.XML_ExpectedEndOfXml;
            ...
            try
            {
                ...
                if (this.CurrentToken.Kind == SyntaxKind.GreaterThanToken)
                {
                    ...
                    try
                    {
                        ...
                        {
                            this.ResetMode(saveMode);
                            lessThanSlash = this.WithXmlParseError(lessThanSlash, XmlParseErrorCode.XML_EndTagExpected,
                                name.ToString());
                            endName = SyntaxFactory.XmlName(prefix: null,
                        ...
                        else
                        {
                            ...
                            {
                                endName = this.WithXmlParseError(endName, XmlParseErrorCode.XML_ElementTypeMatch,
                                    endName.ToString(), name.ToString());
                            }
                ...
                else
                {
                    ...
                    {
                        slashGreater = this.WithXmlParseError(slashGreater, XmlParseErrorCode.XML_ExpectedEndOfTag,
                            name.ToString());
                    }
        ...
        private XmlAttributeSyntax ParseXmlAttribute(XmlNameSyntax elementName)
        {
            ...
            if (hasNoPrefix &&
                DocumentationCommentXmlNames.AttributeEquals(attrNameText,
        ...
        private static bool XmlElementSupportsNameAttribute(XmlNameSyntax elementName)
        {
            ...
            return
                DocumentationCommentXmlNames.ElementEquals(localName,
        ...
        private bool IsVerbatimCref()
        {
                ...
                current.ValueText != ":")
            {
                ...
                if ((current.Kind == SyntaxKind.XmlTextLiteralToken ||
        ...
        private void ParseXmlAttributeText(out SyntaxToken startQuote, SyntaxListBuilder<SyntaxToken> textTokens,
            out SyntaxToken endQuote)
        {
            startQuote = ParseXmlAttributeStartQuote();
            SyntaxKind quoteKind = startQuote.Kind;
        ...
        private XmlNameSyntax ParseXmlName()
        {
            ...
            if (this.CurrentToken.Kind == SyntaxKind.ColonToken)
            {
                ...
                if (prefixTrailingWidth > 0 || colonLeadingWidth > 0)
                {
                    ...
                    colon = WithAdditionalDiagnostics(colon,
                        new XmlSyntaxDiagnosticInfo(offset, width, XmlParseErrorCode.XML_InvalidWhitespace));
                }
                ...
                if (colonTrailingWidth > 0 || localNameLeadingWidth > 0)
                {
                    ...
                    id = WithAdditionalDiagnostics(id,
                        new XmlSyntaxDiagnosticInfo(offset, width, XmlParseErrorCode.XML_InvalidWhitespace));
        ...
        private XmlProcessingInstructionSyntax ParseXmlProcessingInstruction()
        {
            ...
            this.ResetMode(saveMode);
            return SyntaxFactory.XmlProcessingInstruction(startProcessingInstructionToken, name, textTokens,
            ...
            int offset, int length)
        {
            ...
            if (InCref)
            {
                ...
                SyntaxDiagnosticInfo crefInfo = new SyntaxDiagnosticInfo(rawInfo.Offset, rawInfo.Width,
            ...
            switch (expected)
            {
                ...
                default:
                    return new XmlSyntaxDiagnosticInfo(offset, length, XmlParseErrorCode.XML_InvalidToken,
        ...
        {
            NameEx name = ParseCrefName(typeArgumentsMustBeIdentifiers: true);
            CrefParameterListSyntax parameters = ParseCrefParameterList();
        ...
        private OperatorMemberCrefSyntax ParseOperatorMemberCref()
        {
            ...
            else
            {
                ...
                if (SyntaxFacts.IsUnaryOperatorDeclarationToken(CurrentToken.Kind) ||
                ...
                SyntaxDiagnosticInfo rawInfo =
                    new SyntaxDiagnosticInfo(offset, width, ErrorCode.ERR_OvlOperatorExpected);
                SyntaxDiagnosticInfo crefInfo =
                    new SyntaxDiagnosticInfo(offset, width, ErrorCode.WRN_ErrorOverride, rawInfo, rawInfo.Code);
                ...
                CurrentToken.GetLeadingTriviaWidth() == 0)
            {
                ...
                else if (CurrentToken.Kind == SyntaxKind.GreaterThanEqualsToken)
                {
                    ...
                    SyntaxDiagnosticInfo rawInfo =
                        new SyntaxDiagnosticInfo(offset, width, ErrorCode.ERR_OvlOperatorExpected);
                    SyntaxDiagnosticInfo crefInfo = new SyntaxDiagnosticInfo(offset, width, ErrorCode.WRN_ErrorOverride,
        ...
        private NameEx ParseCrefName(bool typeArgumentsMustBeIdentifiers)
        {
        ...
        private TypeEx ParseCrefTypeHelper(bool typeArgumentsMustBeIdentifiers, bool checkForMember = false)
        {
            ...
            if (SyntaxFacts.IsPredefinedType(CurrentToken.Kind))
            {
                ...
                // have nested types.
                return SyntaxFactory.PredefinedTypeEx(EatToken());
            }
