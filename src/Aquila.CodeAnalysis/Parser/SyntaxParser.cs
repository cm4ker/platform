...
namespace Microsoft.CodeAnalysis.CSharp.Syntax.InternalSyntax
{
    ...
    internal abstract partial class SyntaxParser : IDisposable
    {
        ...
        protected virtual SyntaxDiagnosticInfo GetExpectedTokenError(SyntaxKind expected, SyntaxKind actual, int offset,
            int width)
        {
        ...
        protected TNode AddError<TNode>(TNode node, ErrorCode code, params object[] args) where TNode : GreenNode
        {
            ...
            if (token != null && token.ContainsSkippedText)
            {
                ...
                offset = token
                    .GetLeadingTriviaWidth(); // Should always be zero, but at least we'll do something sensible if it's not.
                Debug.Assert(offset == 0,
            ...
            where TNode : CSharpSyntaxNode
        {
            ...
            if (lastToken != null) //will be null if all tokens are missing
            {
                ...
                offset += lastToken
                    .GetLeadingTriviaWidth(); //advance past last token leading trivia - now at start of last token
        ...
        internal SyntaxToken AddSkippedSyntax(SyntaxToken target, GreenNode skippedSyntax, bool trailing)
        {
                    ...
                    if (token.Width > 0)
                    {
                        ...
                        if (leadingWidth > 0)
                        {
                            ...
                            for (int i = 0; i < tokenDiagnostics.Length; i++)
                            {
                                ...
                                tokenDiagnostics[i] = new SyntaxDiagnosticInfo(d.Offset - leadingWidth, d.Width,
                                    (ErrorCode)d.Code, d.Arguments);
                            }
                        }
            ...
            else
            {
                ...
                if (triviaWidth > 0)
                {
                    ...
                    for (int i = 0; i < targetDiagnostics.Length; i++)
                    {
                        ...
                        targetDiagnostics[i] = new SyntaxDiagnosticInfo(d.Offset + triviaWidth, d.Width,
                            (ErrorCode)d.Code, d.Arguments);
                    }
                }
            ...
            if (diagnostic != null)
            {
                ...
                target = WithAdditionalDiagnostics(target,
                    new SyntaxDiagnosticInfo(newOffset, diagnostic.Width, (ErrorCode)diagnostic.Code,
        ...
        protected static SyntaxToken ConvertToKeyword(SyntaxToken token)
        {
            if (token.Kind != token.ContextualKind)
            {
                var kw = token.IsMissing
                    ? SyntaxFactory.MissingToken(token.LeadingTrivia.Node, token.ContextualKind,
        ...
        protected static SyntaxToken ConvertToIdentifier(SyntaxToken token)
        {
            ...
            return SyntaxToken.Identifier(token.Kind, token.LeadingTrivia.Node, token.Text, token.ValueText,
                token.TrailingTrivia.Node);
        }
