...
namespace Aquila.CodeAnalysis.Syntax.InternalSyntax
{
    ...
    internal partial class Lexer : AbstractLexer
    {
        ...
        public Lexer(SourceText text, AquilaParseOptions options, bool allowPreprocessorDirectives = true,
            ...
            else
            {
                switch (info.Kind)
                {
                    case SyntaxKind.IdentifierToken:
                        token = SyntaxFactory.Identifier(info.ContextualKind, leadingNode, info.Text, info.StringValue,
                            trailingNode);
                        break;
                    ...
                    case SyntaxKind.StringLiteralToken:
                        token = SyntaxFactory.Literal(leadingNode, info.Text, info.Kind, info.StringValue,
                            trailingNode);
                        break;
...
#nullable disable
        ...
        private void ScanNumericLiteralSingleInteger(ref bool underscoreInWrongPlace, ref bool usedUnderscore,
            ref bool firstCharWasUnderscore, bool isHex, bool isBinary)
        {
        ...
        private bool ScanNumericLiteral(ref TokenInfo info)
        {
            ...
            if (isHex || isBinary)
            {
                ...
                ScanNumericLiteralSingleInteger(ref underscoreInWrongPlace, ref usedUnderscore,
                    ref firstCharWasUnderscore, isHex, isBinary);
            ...
            {
                ScanNumericLiteralSingleInteger(ref underscoreInWrongPlace, ref usedUnderscore,
                    ref firstCharWasUnderscore, isHex: false, isBinary: false);
        ...
        {
            int result;
            if (!Int32.TryParse(text, isHex ? NumberStyles.AllowHexSpecifier : NumberStyles.None,
        ...
        private ulong GetValueUInt64(string text, bool isHex, bool isBinary)
        {
            ...
            else if (!UInt64.TryParse(text, isHex ? NumberStyles.AllowHexSpecifier : NumberStyles.None,
                         CultureInfo.InvariantCulture, out result))
            {
                //we've already lexed the literal, so the error must be from overflow
                this.AddError(MakeError(ErrorCode.ERR_IntOverflow));
        ...
        private decimal GetValueDecimal(string text, int start, int end)
        {
            ...
            decimal result;
            if (!decimal.TryParse(text, NumberStyles.AllowDecimalPoint | NumberStyles.AllowExponent,
                    CultureInfo.InvariantCulture, out result))
            {
                //we've already lexed the literal, so the error must be from overflow
        ...
        private bool ScanIdentifier_SlowPath(ref TokenInfo info)
        {
            ...
            if (_identLen > 0)
            {
                ...
                if (isObjectAddress)
                {
                    ...
                    Debug.Assert(string.Equals(info.Text.Substring(0, objectAddressOffset + 1), "@0x",
                        StringComparison.OrdinalIgnoreCase));
                    var valueText = TextWindow.Intern(_identBuffer, objectAddressOffset,
            ...
            while (true)
            {
                ...
                switch (consumedChar)
                {
                    case '\\':
                        ...
                        {
                            Debug.Assert(consumedSurrogate == SlidingTextWindow.InvalidCharacter,
                                "Since consumedChar == '\\'");
                    ...
                    {
                        // This is the 'expensive' call
                        if (_identLen == 0 && consumedChar > 127 &&
                            SyntaxFacts.IsIdentifierStartCharacter(consumedChar))
                        {
                            break;
                        }
                        else if (_identLen > 0 && consumedChar > 127 &&
        ...
        private bool ScanMultiLineComment(out bool isTerminated)
        {
            if (TextWindow.PeekChar() == '/' && TextWindow.PeekChar(1) == '*')
            {
                ...
                while (true)
                {
                    if ((ch = TextWindow.PeekChar()) == SlidingTextWindow.InvalidCharacter &&
            ...
            while (true)
            {
                ...
                switch (ch)
                {
                    ...
                    case '#':
                        ...
                        TextWindow.Reset(
                            lastLineStart); // reset so directive parser can consume the starting whitespace on this line
                        return TextWindow.Width > 0 ? SyntaxFactory.DisabledText(TextWindow.GetText(false)) : null;
                    case '\r':
                    case '\n':
                        this.ScanEndOfLine();
        ...
        private SyntaxToken LexXmlElementTagToken()
        {
            ...
            if (errors == null && tagInfo.ContextualKind == SyntaxKind.None &&
        ...
        private bool ScanXmlCrefToken(ref TokenInfo info)
        {
            ...
            switch (consumedChar)
            {
                ...
                case '<':
                    info.Text = TextWindow.GetText(intern: false);
                    this.AddError(XmlParseErrorCode.XML_LessThanInAttributeValue,
                        info.Text); //ErrorCode.WRN_XMLParseError
                    return true;
            ...
            else if (this.LocationIs(XmlDocCommentLocation.Start) || this.LocationIs(XmlDocCommentLocation.Exterior))
            {
                ...
                while (true)
                {
                    ...
                    switch (ch)
                    {
                        ...
                        case '/':
                            if (this.StyleIs(XmlDocCommentStyle.SingleLine) && TextWindow.PeekChar(1) == '/' &&
