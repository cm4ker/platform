...
namespace Aquila.CodeAnalysis.Syntax.InternalSyntax
{
    ...
    internal sealed class SlidingTextWindow : IDisposable
    {
        ...
        private static readonly ObjectPool<char[]> s_windowPool =
            new ObjectPool<char[]>(() => new char[DefaultWindowLength]);
        ...
        public char NextCharOrUnicodeEscape(out char surrogateCharacter, out SyntaxDiagnosticInfo info)
        {
            ...
            if (ch == '\\')
            {
                ...
                if (ch2 == 'U' || ch2 == 'u')
                {
                    return this.ScanUnicodeEscape(peek: false, surrogateCharacter: out surrogateCharacter,
                        info: out info);
                }
            }
        ...
        public string GetText(int position, int length, bool intern)
        {
            ...
            switch (length)
            {
                ...
                case 3:
                    if (_characterWindow[offset] == '/' && _characterWindow[offset + 1] == '/' &&
