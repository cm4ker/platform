...
namespace Aquila.CodeAnalysis.Semantics
{
    ...
    internal static class VariableReferenceExtensions
    {
            ...
            BoundAccess access)
        {
            ...
            else if (access.EnsureObject) // : object
            {
                ...
                else
                {
                    ...
                    {
                        if (type == cg.CoreTypes.Object && /*cg.TypeRefContext.IsNull(_thint) &&*/ place.HasAddress
                        ) // TODO: only if place can be NULL
                        {
            ...
            TypeSymbol receiver)
        {
            ...
            {
                var statics =
                    symbol is FieldSymbol field
                        ? field.ContainingStaticsHolder()
                        : null; // in case field is a PHP static field
                if (statics != null)
                {
                ...
                else
                {
                    if (receiver == null)
                    {
                        throw cg.NotImplementedException(
                            $"Non-static field {symbol.ContainingType.Name}::${symbol.MetadataName} accessed statically!");
                    }
                    else
                    {
    ...
    class LocalVariableReference : IVariableReference
    {
        ...
        TypeSymbol LoadVariablesArray(CodeGenerator cg)
        {
            ...
            {
                Debug.Assert(cg.TemporalLocalsPlace != null,
    ...
    class ThisVariableReference : LocalVariableReference
    {
        ...
        public override TypeSymbol EmitLoadValue(CodeGenerator cg, ref LhsStack lhs, BoundAccess access)
        {
            ...
            {
                return (new ParamPlace(cg.GeneratorStateMachineMethod.ThisParameter))
                    .EmitLoadValue(cg, ref lhs, access);
        ...
        public IPlace Place
        {
            get
            {
                if (Receiver == null)
                {
                    ...
                    if (!Field.IsStatic)
                    {
                        Debug.Assert(Field.ContainingStaticsHolder() != null,
                            "Field is non-static and does not have the receiver instance!");
