...
//         }
//
//         void EmitFieldsCctor(PEModuleBuilder module)
//         {
...
//                 lock (cctor)
//                 {
//                     using (var cg = new CodeGenerator(cctor, module, DiagnosticBag.GetInstance(),
//                         module.Compilation.Options.OptimizationLevel, false, this, null, thisPlace: null)
//                     {
//                         CallerType = this,
...
//         }
//
//         void EmitPhpCallable(PEModuleBuilder module, DiagnosticBag diagnostics)
//         {
...
//             //
//             var invoke = new SynthesizedMethodSymbol(this, iphpcallable.FullName + ".Invoke", false, true,
//                 DeclaringCompilation.CoreTypes.PhpValue, isfinal: true)
//             {
//                 ExplicitOverride = (MethodSymbol) iphpcallable.Symbol.GetMembers("Invoke").Single(),
//                 ForwardedCall = __invoke,
//             };
//             invoke.SetParameters(
//                 new SpecialParameterSymbol(invoke, DeclaringCompilation.CoreTypes.Context,
//                     SpecialParameterSymbol.ContextName, 0),
//                 new SynthesizedParameterSymbol(invoke,
//                     ArrayTypeSymbol.CreateSZArray(ContainingAssembly, DeclaringCompilation.CoreTypes.PhpValue.Symbol),
//                     1, RefKind.None, name: "arguments", isParams: true));
//
//             module.SetMethodBody(invoke, MethodGenerator.GenerateMethodBody(module, invoke, il =>
//             {
//                 var cg = new CodeGenerator(il, module, diagnostics, module.Compilation.Options.OptimizationLevel, false,
//                     this, new ParamPlace(invoke.Parameters[0]), new ArgPlace(this, 0))
//                 {
//                     CallerType = this,
//                 };
//
...
//             //
//             var tophpvalue = new SynthesizedMethodSymbol(this, iphpcallable.FullName + ".ToPhpValue", false, true,
//                 DeclaringCompilation.CoreTypes.PhpValue, isfinal: true)
//             {
...
//                 method.OverriddenMethod.ContainingType.TypeKind != TypeKind.Interface)
//             {
//                 return true;
//             }
//
...
//         }
//
//         void EmitDisposable(PEModuleBuilder module, DiagnosticBag diagnostics)
//         {
...
//             // IDisposable.Dispose()
//             //
//             var dispose = new SynthesizedMethodSymbol(this, idisposable.GetFullName() + ".Dispose", false, true,
//                 DeclaringCompilation.CoreTypes.Void, isfinal: true)
//             {
//                 ExplicitOverride =
//                     (MethodSymbol) DeclaringCompilation.GetSpecialTypeMember(SpecialMember.System_IDisposable__Dispose),
...
//                 var disposedField = cg.Module.SynthesizedManager.GetOrCreateSynthesizedField(this,
//                     DeclaringCompilation.CoreTypes.Boolean, WellKnownPchpNames.SynthesizedDisposedFieldName,
...
//         }
//
//         void EmitPhpCtors(ImmutableArray<MethodSymbol> instancectors, PEModuleBuilder module,
...
//
//                     var cg = new CodeGenerator(il, module, diagnostics, module.Compilation.Options.OptimizationLevel,
//                         false, this, new ParamPlace(ctor.Parameters[0]), new ArgPlace(this, 0))
//                     {
//                         CallerType = this,
...
//                         foreach (var f in this.GetMembers().OfType<IPhpPropertySymbol>()
//                             .Where(f => f.FieldKind == PhpPropertyKind.InstanceField))
//                         {
//                             Debug.Assert(f.ContainingStaticsHolder == null);
//                             f.EmitInit(cg);
//                         }
...
//         }
//
//         void EmitParameterlessCtor(SynthesizedPhpCtorSymbol ctor, ILBuilder il, PEModuleBuilder module,
//             DiagnosticBag diagnostics)
//         {
//             Debug.Assert(ctor.BaseCtor != null);
...
//             var extensions =
//                 (NamedTypeSymbol) module.Compilation.GetTypeByMetadataName("Pchp.Core.Utilities.ContextExtensions");
...
//             //
//             var cg = new CodeGenerator(il, module, diagnostics, module.Compilation.Options.OptimizationLevel, false,
//                 this, new PropertyPlace(null, ctxfield), new ArgPlace(this, 0))
//             {
//                 CallerType = this,
//                 ContainingFile = ContainingFile,
//             };
...
//         }
//
//         void EmitToString(PEModuleBuilder module)
//         {
...
//             }
//
//             var __tostring = this.GetMembersByPhpName(Name.SpecialMethodNames.Tostring.Value).OfType<MethodSymbol>()
//                 .FirstOrDefault();
...
//                     m => OverrideHelper.SignaturesMatch(m,
//                         (MethodSymbol) DeclaringCompilation.GetSpecialTypeMember(SpecialMember
...
//                 var tostring = new SynthesizedMethodSymbol(this, WellKnownMemberNames.ObjectToString, false, true,
//                     DeclaringCompilation.CoreTypes.String, Accessibility.Public, isfinal: false, phphidden: true)
//                 {
...
//                 {
//                     var thisPlace = new ArgPlace(this, 0);
//                     var cg = new CodeGenerator(il, module, DiagnosticBag.GetInstance(),
//                         module.Compilation.Options.OptimizationLevel, false, this,
//                         new FieldPlace(thisPlace, this.ContextStore, module), thisPlace);
//
//                     if (__tostring != null)
//                     {
//                         // __tostring().ToString()
//                         cg.EmitConvert(cg.EmitForwardCall(__tostring, tostring, callvirt: true), 0,
//                             tostring.ReturnType);
//                     }
//                     else
//                     {
...
//                             cg.CoreTypes.PhpException.Method("ObjectToStringNotSupported", cg.CoreTypes.Object)));
//
//                         // return ""
...
//         }
//
//         void EmitTraitImplementations(PEModuleBuilder module)
//         {
...
//                             traitInstancePlace =
//                                 new FieldPlace(thisPlace, t.TraitInstanceField, module); // this.<>trait
//                         }
//
//                         using (var cg = new CodeGenerator(il, module, DiagnosticBag.GetInstance(),
//                             module.Compilation.Options.OptimizationLevel, false, this, ctxPlace, thisPlace)
//                         {
//                             CallerType = this,
//                         })
//                         {
//                             var forwarded_type = cg.EmitForwardCall(m.ForwardedCall, m,
//                                 thisPlaceExplicit: traitInstancePlace);
...
//         /// </summary>
//         public void FinalizeMethodTable(PEModuleBuilder module, DiagnosticBag diagnostics)
//         {
...
//
//                 if (ReferenceEquals(info.OverrideCandidate?.ContainingType,
//                         this) || // candidate not matching exactly the signature in this type
...
//                         if (info.IsUnresolvedAbstract && info.ImplementsInterface && this.IsAbstract &&
//                             !this.IsInterface)
//                         {
...
//                 }
//
//                 if (info.Method is SynthesizedMethodSymbol && info.Method.ContainingType == this &&
//                     info.Method.IsAbstract)
//                 {
...
//         }
//
//         void GenerateFieldAccessorProperty(PEModuleBuilder module, DiagnosticBag diagnostics,
//             SourceFieldSymbol srcf, PropertySymbol paccessor)
//         {
//             //
