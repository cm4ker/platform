...
namespace Aquila.CodeAnalysis.Symbols
{
        ...
        Cci.IMethodDefinition
    {
        ...
        {
            get { return this.IsGenericMethod; }
        }

        ushort Cci.ISignature.ParameterCount
        {
            get { return (ushort)this.Parameters.Length; }
        }
        ...
        private Cci.IMethodDefinition ResolvedMethodImpl(EmitContext context)
        {
            ...
            if (this.IsDefinition && // can't be generic instantiation
                object.ReferenceEquals(this.ContainingModule,
                    moduleBeingBuilt.SourceModule)) // must be declared in the module we are building
            {
        ...
        {
            get { return this.ReturnTypeCustomModifiers.As<Cci.ICustomModifier>(); }
        }

        public virtual bool ReturnValueIsByRef
        {
            get { return RefKind != RefKind.None; }
        }
        ...
        Cci.IMethodReference Cci.IGenericMethodInstanceReference.GetGenericMethod(EmitContext context)
        {
            ...
            if (!PEModuleBuilder.IsGenericType(this.ContainingType))
            {
                ...
                return ((PEModuleBuilder)context.Module).Translate(
                    this.OriginalDefinition, context.Diagnostics, true);
            }
            ...
            get
            {
                ...
                return this.IsSealed ||
                       (this.IsMetadataVirtual() &&
                        !(this.IsVirtual || this.IsOverride || this.IsAbstract ||
        ...
        bool Cci.IMethodDefinition.IsVirtual
        {
            get
            {
                ...
                Debug.Assert(isvirt || (!IsMetadataFinal && !IsMetadataNewSlot()),
                    "Method marked Final or NewSlot or CheckAccessOnOverride but not Virtual.");
                Debug.Assert(!isvirt || this.MethodKind != MethodKind.Constructor,
                    $"Virtual Instance Constructor in {ContainingType?.Name} - instance constructor cannot be marked Virtual.");
                return isvirt;
            }
        }
        ...
        {
            get { return (Cci.INamespace)ContainingNamespace; }
        }
