...
{
    internal sealed class MethodCompiler : AquilaSymbolVisitor<TypeCompilationState, object>
    {
        ...
        internal MethodCompiler(AquilaCompilation compilation, PEModuleBuilder moduleBeingBuiltOpt, bool emittingPdb,
            bool emitTestCoverageData, bool hasDeclarationErrors, bool emitMethodBodies,
            BindingDiagnosticBag diagnostics, Predicate<Symbol> filterOpt, CancellationToken cancellationToken)
        {
            ...
            if (emittingPdb || emitTestCoverageData)
            {
                _debugDocumentProvider = (path, basePath) =>
                    moduleBeingBuiltOpt.DebugDocumentsBuilder.GetOrAddDebugDocument(path, basePath,
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                entryPoint = GetEntryPoint(compilation, moduleBeingBuiltOpt, hasDeclarationErrors, emitMethodBodies,
        ...
        private static MethodSymbol GetEntryPoint(AquilaCompilation compilation, PEModuleBuilder moduleBeingBuilt,
            bool hasDeclarationErrors, bool emitMethodBodies, BindingDiagnosticBag diagnostics,
            CancellationToken cancellationToken)
        {
            return (MethodSymbol)compilation.GetEntryPoint(cancellationToken);
        }
        ...
        private void CompileNamedType(NamedTypeSymbol containingType)
        {
            ...
            for (int memberOrdinal = 0; memberOrdinal < members.Length; memberOrdinal++)
            {
                ...
                switch (member.Kind)
                {
                    ...
                    case SymbolKind.Method:
                    {
                        ...
                        CompileMethod(method, memberOrdinal, compilationState);
            ...
            if (_moduleBeingBuiltOpt != null && !hasStaticConstructor &&
                !processedStaticInitializers.BoundInitializers.IsDefaultOrEmpty)
            {
                ...
                {
                    CompileMethod(method, -1, ref processedStaticInitializers, synthesizedSubmissionFields,
                        compilationState);
            ...
            if (!hasStaticConstructor &&
                ...
                containingType is
                {
                    IsImplicitlyDeclared: false, TypeKind: TypeKind.Class or TypeKind.Struct or TypeKind.Interface
                } &&
            ...
            if (scriptCtor != null && compilationState.Emitting)
            {
                ...
                CompileMethod(scriptCtor, scriptCtorOrdinal, ref processedInitializers, synthesizedSubmissionFields,
                    compilationState);
            ...
            BindingDiagnosticBag diagnostics)
        {
            ...
            foreach (Cci.IMethodDefinition definition in privateImplClass.GetMethods(
                         new EmitContext(_moduleBeingBuiltOpt, null, diagnostics.DiagnosticBag, metadataOnly: false,
            ...
            try
            {
                foreach (var methodWithBody in synthesizedMethods)
                {
                    ...
                    try
                    {
                        ...
                        BoundStatement loweredBody = IteratorRewriter.Rewrite(methodWithBody.Body, method,
                            methodOrdinal, variableSlotAllocatorOpt, compilationState, diagnosticsThisMethod,
                            out iteratorStateMachine);
                        StateMachineTypeSymbol stateMachine = iteratorStateMachine;

                        if (!loweredBody.HasErrors)
                        {
                            AsyncStateMachine asyncStateMachine;
                            loweredBody = AsyncRewriter.Rewrite(loweredBody, method, methodOrdinal,
                                variableSlotAllocatorOpt, compilationState, diagnosticsThisMethod,
        ...
        /// </summary>
        private void CompileSynthesizedExplicitImplementations(SourceMemberContainerTypeSymbol sourceTypeSymbol,
            TypeCompilationState compilationState)
        {
            ...
            if (!_globalHasErrors)
            {
                ...
                foreach (var synthesizedExplicitImpl in sourceTypeSymbol
                             .GetSynthesizedExplicitImplementations(_cancellationToken).ForwardingMethods)
                {
                    ...
                    _moduleBeingBuiltOpt.AddSynthesizedDefinition(sourceTypeSymbol,
            ...
            TypeCompilationState compilationState)
        {
            ...
            if ((object)synthesizedAccessor != null && !_globalHasErrors)
            {
                ...
                _moduleBeingBuiltOpt.AddSynthesizedDefinition(sourceProperty.ContainingType,
        ...
        private void CompileFieldLikeEventAccessor(SourceEventSymbol eventSymbol, bool isAddMethod)
        {
            ...
            try
            {
                BoundBlock boundBody = MethodBodySynthesizer.ConstructFieldLikeEventAccessorBody(eventSymbol,
                    isAddMethod, _compilation, diagnosticsThisMethod);
        ...
        private void CompileMethod(MethodSymbol methodSymbol, int methodOrdinal,
            TypeCompilationState compilationState)
        {
            _cancellationToken.ThrowIfCancellationRequested();
            ...
            try
            {
                ...
                else if (methodSymbol.IsScriptInitializer)
                {
                    ...
                    var initializerStatements = InitializerRewriter.RewriteScriptInitializer(
                        processedInitializers.BoundInitializers, (SynthesizedInteractiveInitializerMethod)methodSymbol,
                        out hasTrailingExpression);

                    // the lowered script initializers should not be treated as initializers anymore but as a method body:
                    body = BoundBlock.SynthesizedNoLocals(initializerStatements.Syntax,
                        initializerStatements.Statements);
                    ...
                    DefiniteAssignmentPass.Analyze(_compilation, methodSymbol, initializerStatements, unusedDiagnostics,
                        requireOutParamsAssigned: false);
                    DiagnosticsPass.IssueDiagnostics(_compilation, initializerStatements,
                        BindingDiagnosticBag.Discarded, methodSymbol);
                ...
                else
                {
                    ...
                    includeNonEmptyInitializersInBody = includeInitializersInBody &&
                    ...
                    if (includeNonEmptyInitializersInBody && processedInitializers.LoweredInitializers == null)
                    {
                        analyzedInitializers =
                            InitializerRewriter.RewriteConstructor(processedInitializers.BoundInitializers,
                                methodSymbol);
                        processedInitializers.HasErrors =
                            processedInitializers.HasErrors || analyzedInitializers.HasAnyErrors;
                    }
                    ...
                    body = BindMethodBody(
                        ...
                        ReportNullableDiagnostics,
                        includesFieldInitializers: includeInitializersInBody &&
                                                   !processedInitializers.BoundInitializers.IsEmpty,
                        out importChain,
                    ...
                    if (includeNonEmptyInitializersInBody && processedInitializers.LoweredInitializers == null)
                    {
                        if (body != null &&
                            ((methodSymbol.ContainingType.IsStructType() && !methodSymbol.IsImplicitConstructor) ||
                             methodSymbol is SynthesizedRecordConstructor || _emitTestCoverageData))
                        {
                            if (_emitTestCoverageData && methodSymbol.IsImplicitConstructor)
                            {
                                ...
                                // are analyzed here.
                                DefiniteAssignmentPass.Analyze(_compilation, methodSymbol, analyzedInitializers,
                                    diagsForCurrentMethod.DiagnosticBag, requireOutParamsAssigned: false);
                            }

                            // In order to get correct diagnostics, we need to analyze initializers and the body together.
                            body = body.Update(body.Locals, body.LocalFunctions,
                                body.Statements.Insert(0, analyzedInitializers));
                        ...
                        else
                        {
                            ...
                            DefiniteAssignmentPass.Analyze(_compilation, methodSymbol, analyzedInitializers,
                                diagsForCurrentMethod.DiagnosticBag, requireOutParamsAssigned: false);
                            DiagnosticsPass.IssueDiagnostics(_compilation, analyzedInitializers, diagsForCurrentMethod,
                                methodSymbol);
                        }
                    }
                }
                ...
                Debug.Assert(processedInitializers.BoundInitializers.IsDefaultOrEmpty ||
                             processedInitializers.FirstImportChain != null);
#endif
                ...
                if (body != null)
                {
                    flowAnalyzedBody = FlowAnalysisPass.Rewrite(methodSymbol, body, diagsForCurrentMethod.DiagnosticBag,
                        hasTrailingExpression: hasTrailingExpression, originalBodyNested: originalBodyNested);
                }

                bool hasErrors = _hasDeclarationErrors || diagsForCurrentMethod.HasAnyErrors() ||
                                 processedInitializers.HasErrors;
                ...
                {
                    actualDiagnostics = new ImmutableBindingDiagnostic<AssemblySymbol>(
                        sourceMethod.SetDiagnostics(actualDiagnostics.Diagnostics, out diagsWritten),
                        actualDiagnostics.Dependencies);
                }
                ...
                if (diagsWritten && !methodSymbol.IsImplicitlyDeclared && _compilation.EventQueue != null)
                {
                        ...
                        _compilation.SemanticModelProvider is CachingSemanticModelProvider cachingSemanticModelProvider)
                    {
                        ...
                        semanticModelWithCachedBoundNodes =
                            (SyntaxTreeSemanticModel)cachingSemanticModelProvider.GetSemanticModel(syntax.SyntaxTree,
                                _compilation);
                    ...
                    _compilation.EventQueue.TryEnqueue(new SymbolDeclaredCompilationEvent(_compilation,
                ...
                try
                {
                    ...
                    hasErrors = hasErrors || (hasBody && loweredBodyOpt.HasErrors) ||
                    ...
                    {
                        Debug.Assert(
                            !(methodSymbol.IsImplicitInstanceConstructor && methodSymbol.ParameterCount == 0) ||
                        ...
                        if (methodSymbol.IsScriptConstructor)
                        {
                            boundStatements = MethodBodySynthesizer.ConstructScriptConstructorBody(loweredBodyOpt,
                                methodSymbol, previousSubmissionFields, _compilation);
                        }
                        else
                        {
                            ...
                            if (includeNonEmptyInitializersInBody)
                            {
                                if (processedInitializers.LoweredInitializers.Kind == BoundKind.StatementList)
                                {
                                    ...
                                    boundStatements = Enumerable.Concat(boundStatements, lowered.Statements);
                                }
                                else
                                {
                            ...
                            if (hasBody)
                            {
                                boundStatements = Enumerable.Concat(boundStatements,
                                    ImmutableArray.Create(loweredBodyOpt));
                            }
                        }

                        if (_emitMethodBodies && (!(methodSymbol is SynthesizedStaticConstructor cctor) ||
                                                  cctor.ShouldEmit(processedInitializers.BoundInitializers)))
                        {
                            ...
                            _moduleBeingBuiltOpt.SetMethodBody(methodSymbol.PartialDefinitionPart ?? methodSymbol,
                                emittedBody);
                        }
                    }
            ...
            out StateMachineTypeSymbol stateMachineTypeOpt)
        {
            ...
            try
            {
                ...
                {
                    lazyVariableSlotAllocator =
                        compilationState.ModuleBuilderOpt.TryCreateVariableSlotAllocator(method, method,
                ...
                BoundStatement bodyWithoutIterators = IteratorRewriter.Rewrite(bodyWithoutLambdas, method,
                    methodOrdinal, lazyVariableSlotAllocator, compilationState, diagnostics,
                ...
                BoundStatement bodyWithoutAsync = AsyncRewriter.Rewrite(bodyWithoutIterators, method, methodOrdinal,
                    lazyVariableSlotAllocator, compilationState, diagnostics,
            ...
            SynthesizedEntryPointSymbol.AsyncForwardEntryPoint entryPointOpt)
        {
            ...
            Debug.Assert(!diagnostics.HasAnyErrors(),
                "Running code generator when errors exist might be dangerous; code generator not expecting errors");

            var compilation = moduleBuilder.Compilation;
            var localSlotManager = new LocalSlotManager(variableSlotAllocatorOpt);
            var optimizations = compilation.Options.OptimizationLevel.AsOptimizationLevel();

            ILBuilder builder = new ILBuilder(moduleBuilder, localSlotManager, optimizations, true);
            bool hasStackalloc;
            var diagnosticsForThisMethod =
                BindingDiagnosticBag.GetInstance(withDiagnostics: true, diagnostics.AccumulatesDependencies);
            try
            {
                StateMachineMoveNextBodyDebugInfo moveNextBodyDebugInfoOpt = null;

                var codeGen = new CodeGen.CodeGenerator(method, block, builder, moduleBuilder,
                    diagnosticsForThisMethod.DiagnosticBag, optimizations, emittingPdb);
                ...
                {
                    codeGen.Generate(out int asyncCatchHandlerOffset, out var asyncYieldPoints,
                        out var asyncResumePoints, out hasStackalloc);
                ...
                if (optimizations == OptimizationLevel.Debug && (object)stateMachineTypeOpt != null)
                {
                    ...
                    GetStateMachineSlotDebugInfo(moduleBuilder, moduleBuilder.GetSynthesizedFields(stateMachineTypeOpt),
                        variableSlotAllocatorOpt, diagnosticsForThisMethod, out stateMachineHoistedLocalSlots,
                        out stateMachineAwaiterSlots);
                    Debug.Assert(!diagnostics.HasAnyErrors());
                }
                ...
                return new MethodBody(
                    ...
                    variableSlotAllocatorOpt?.MethodId ??
                    new DebugId(methodOrdinal, moduleBuilder.CurrentGenerationOrdinal),
                    localVariables,
        ...
        private static void GetStateMachineSlotDebugInfo(
            PEModuleBuilder moduleBuilder,
            IEnumerable<Microsoft.Cci.IFieldDefinition> fieldDefs,
            VariableSlotAllocator variableSlotAllocatorOpt,
            BindingDiagnosticBag diagnostics,
            out ImmutableArray<EncHoistedLocalInfo> hoistedVariableSlots,
            out ImmutableArray<Microsoft.Cci.ITypeReference> awaiterSlots)
        {
            var hoistedVariables = ArrayBuilder<EncHoistedLocalInfo>.GetInstance();
            var awaiters = ArrayBuilder<Microsoft.Cci.ITypeReference>.GetInstance();
                    ...
                    )
            {
                ...
                else if (!field.SlotDebugInfo.Id.IsNone)
                {
                    ...
                    hoistedVariables[index] = new EncHoistedLocalInfo(field.SlotDebugInfo,
                        moduleBuilder.EncTranslateLocalVariableType(field.Type, diagnostics.DiagnosticBag));
                }
            }
        ...
        internal static BoundBlock BindMethodBody(MethodSymbol method, TypeCompilationState compilationState,
            BindingDiagnosticBag diagnostics)
        {
            return BindMethodBody(method, compilationState, diagnostics, nullableInitialState: null,
                reportNullableDiagnostics: true, includesFieldInitializers: false, out _, out _, out _);
        }
            ...
            out MethodBodySemanticModel.InitialState forSemanticModel)
        {
            ...
            else if (method is SourceMethodSymbol sourceMethod)
            {
                AquilaSyntaxNode syntaxNode = sourceMethod.SyntaxNode;
                ...
                if (bodyBinder != null)
                {
                    ...
                    BoundNode methodBody =
                        bodyBinder.BindMethodBody(syntaxNode, diagnostics, includesFieldInitializers);
                    ...
                    if (reportNullableDiagnostics)
                    {
                        ...
                        {
                            var isSufficientLangVersion = compilation.LanguageVersion >=
                    ...
                    forSemanticModel = new MethodBodySemanticModel.InitialState(syntaxNode, methodBodyForSemanticModel,
                        bodyBinder, snapshotManager, remappedSymbols);
                    ...
                    switch (methodBody.Kind)
                    {
                        case BoundKind.ConstructorMethodBody:
                            ...
                            {
                                ReportCtorInitializerCycles(method, expressionStatement.Expression, compilationState,
                                    diagnostics);

                                if (body == null)
                                {
                                    body = new BoundBlock(constructor.Syntax, constructor.Locals,
                                        ImmutableArray.Create<BoundStatement>(constructor.Initializer));
                                }
                                else
                                {
                                    body = new BoundBlock(constructor.Syntax, constructor.Locals,
                                        ImmutableArray.Create<BoundStatement>(constructor.Initializer, body));
            ...
            if (reportNullableDiagnostics && method.IsConstructor() && method.IsImplicitlyDeclared &&
        ...
        private static BoundStatement BindImplicitConstructorInitializerIfAny(MethodSymbol method,
            TypeCompilationState compilationState, BindingDiagnosticBag diagnostics)
        {
            ...
            if (method.MethodKind == MethodKind.Constructor && !method.IsExtern)
            {
                ...
                if (initializerInvocation != null)
                {
                    ...
                    var constructorInitializer =
                        new BoundExpressionStatement(initializerInvocation.Syntax, initializerInvocation)
                            { WasCompilerGenerated = method.IsImplicitlyDeclared };
                    Debug.Assert(
                        initializerInvocation.HasAnyErrors || constructorInitializer.IsConstructorInitializer(),
                        "Please keep this bound node in sync with BoundNodeExtensions.IsConstructorInitializer.");
        ...
        private static void ReportCtorInitializerCycles(MethodSymbol method, BoundExpression initializerInvocation,
            TypeCompilationState compilationState, BindingDiagnosticBag diagnostics)
        {
            var ctorCall = initializerInvocation as BoundCall;
            if (ctorCall != null && !ctorCall.HasAnyErrors && ctorCall.Method != method &&
                TypeSymbol.Equals(ctorCall.Method.ContainingType, method.ContainingType,
        ...
        internal static BoundExpression BindImplicitConstructorInitializer(
            MethodSymbol constructor, BindingDiagnosticBag diagnostics, AquilaCompilation compilation)
        {
            ...
            Binder initializerBinder =
                outerBinder.WithAdditionalFlagsAndContainingMemberOrLambda(BinderFlags.ConstructorInitializer,
                    constructor);

            return initializerBinder.BindConstructorInitializer(null, constructor, diagnostics);
        }

        private static SyntaxToken GetImplicitConstructorBodyToken(AquilaSyntaxNode containerNode)
        {
            return ((BaseTypeDeclarationSyntax)containerNode).OpenBraceToken;
        }

        internal static BoundCall GenerateBaseParameterlessConstructorInitializer(MethodSymbol constructor,
            BindingDiagnosticBag diagnostics)
        {
            ...
            Location diagnosticsLocation =
                constructor.Locations.IsEmpty ? NoLocation.Singleton : constructor.Locations[0];
            ...
            {
                diagnostics.Add(ErrorCode.ERR_BadCtorArgCount, diagnosticsLocation, baseType, /*desired param count*/
                    0);
                return null;
            }
        ...
        private static BoundCall GenerateBaseCopyConstructorInitializer(SynthesizedRecordCopyCtor constructor,
            BindingDiagnosticBag diagnostics)
        {
            NamedTypeSymbol containingType = constructor.ContainingType;
            NamedTypeSymbol baseType = containingType.BaseTypeNoUseSiteDiagnostics;
            ...
            MethodSymbol baseConstructor =
                SynthesizedRecordCopyCtor.FindCopyConstructor(baseType, containingType, ref useSiteInfo);
        ...
        private static Microsoft.Cci.DebugSourceDocument CreateDebugDocumentForFile(string normalizedPath)
        {
            return new Microsoft.Cci.DebugSourceDocument(normalizedPath,
                Microsoft.Cci.DebugSourceDocument.CorSymLanguageTypeCSharp);
        }
