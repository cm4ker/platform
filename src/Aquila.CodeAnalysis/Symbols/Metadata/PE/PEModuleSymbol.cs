    ...
    internal sealed class PEModuleSymbol : NonMissingModuleSymbol
    {
        ...
        internal ImmutableArray<CSharpAttributeData> GetAssemblyAttributes()
        {
            if (_lazyAssemblyAttributes.IsDefault)
            {
                ...
                {
                    foreach (var qualifier in Microsoft.Cci.MetadataWriter.dummyAssemblyAttributeParentQualifier)
                    {
                        EntityHandle typerefAssemblyAttributesGoHere =
                                    Module.GetTypeRef(
                                        assemblyMSCorLib,
                                        Microsoft.Cci.MetadataWriter.dummyAssemblyAttributeParentNamespace,
                                        Microsoft.Cci.MetadataWriter.dummyAssemblyAttributeParentName + qualifier);
