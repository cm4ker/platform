...
namespace Aquila.CodeAnalysis.Symbols
{
    ...
    internal class SourceLocalSymbol : Symbol, ILocalSymbol, ILocalSymbolInternal
    {
        ...
        public virtual ITypeSymbol Type
        {
            get
            {
                var tsymbol = DeclaringCompilation.GetTypeFromTypeRef(_routine,
                    _routine.ControlFlowGraph.GetLocalTypeMask(this.Name));
                if (tsymbol.SpecialType == SpecialType.System_Void)
                {
                    tsymbol = DeclaringCompilation.CoreTypes
                        .PhpValue; // temporary workaround for uninitialized variables
                }
