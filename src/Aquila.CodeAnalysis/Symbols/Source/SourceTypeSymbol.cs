...
//                     {
//                         lazyContextField = new SynthesizedFieldSymbol(this,
//                             DeclaringCompilation.CoreTypes.Context.Symbol, SpecialParameterSymbol.ContextName,
...
//                     {
//                         lazyRuntimeFieldsField = new SynthesizedFieldSymbol(this,
//                             DeclaringCompilation.CoreTypes.PhpArray.Symbol, fldname, Accessibility.Internal, false);
//                     }
//
...
//         /// </summary>
//         public IMethodSymbol InstanceConstructorFieldsOnly =>
//             InstanceConstructors.SingleOrDefault(ctor => ctor.IsInitFieldsOnly);
//
...
//                     {
//                         var lazyTraitInstanceField = new SynthesizedFieldSymbol(ContainingType, Symbol,
//                             "<>" + "trait_" + Symbol.Name,
...
//                                         precedence.IgnoredTypes.Select(t => t.QualifiedName.Value)
//                                             .Contains(((IPhpTypeSymbol) declared.SourceMethod.ContainingType
//                                                 .OriginalDefinition).FullName))
//                                     {
//                                         // member was hidden
...
//                                     var qname = a.TraitMemberName.Item1 != null
//                                         ? a.TraitMemberName.Item1.QualifiedName.Value
//                                         : phpt.FullName;
//
//                                     // add an alias to the map:
//                                     if (sourcemembers.TryGetValue(new MemberQualifiedName(qname, membername),
//                                         out MethodSymbol s))
//                                     {
//                                         if (map.TryGetValue(membername, out DeclaredAs declaredas) &&
//                                             declaredas.SourceMethod == s)
//                                         {
...
//                                         };
//
//                                         if (alias.NewModifier.HasValue)
//                                             declaredas.Accessibility = alias.NewModifier.Value.GetAccessibility();
...
//                     }
//
//                     yield return new Synthesized.SynthesizedTraitFieldSymbol(ContainingType,
//                         (FieldSymbol) TraitInstanceField, p);
//                 }
//
//                 // methods
...
//                     if (m.SourceMethod.IsAbstract || (m.SourceMethod.OriginalDefinition is SourceMethodSymbol srcm &&
//                                                       ((MethodDecl) srcm.Syntax).Modifiers.IsAbstract())
//                     ) // TODO: determine abstract method better
//                     {
...
//                 this.Symbol =
//                     symbol.Construct(type is SourceTraitTypeSymbol tt
//                         ? tt.TSelfParameter
//                         : type); // if contained in trait, pass its TSelf
...
//                 {
//                     if (visited.Add(_syntax.BaseClass.ClassName) && this.BaseType is SourceTypeSymbol s &&
//                         s.IsUnreachableChecked(ref visited))
//                     {
//                         return true;
//                     }
//                 }
...
//                 {
//                     if (t.Symbol is SourceTypeSymbol s && visited.Add(s.FullName) &&
//                         s.IsUnreachableChecked(ref visited))
//                     {
//                         return true;
//                     }
...
//
//             //
//             _staticsContainer = new Synthesized.SynthesizedStaticFieldsHolder(this);
//         }
//
//         /// <summary>
...
//
//         private SourceTypeSymbol InitNewSelf(SourceTypeSymbol self, NamedTypeSymbol baseType,
//             ImmutableArray<NamedTypeSymbol> ifacesType, ImmutableArray<TraitUse> traitUses, int version,
//             SourceTypeSymbol nextVersion)
//         {
...
//                     _lazyBaseType =
//                         ((_syntax.MemberAttributes & (PhpMemberAttributes.Static | PhpMemberAttributes.Interface)) ==
//                          0) // not static class nor interface
...
//                         .ToArray();
//
//                     Debug.Assert(tsignature.Length >= 1 &&
//                                  tsignature.Skip(1)
//                                      .All(x => x.Symbol !=
//                                                null)); // all the types (except for the base) have bound symbol (it might be ErrorTypeSymbol but never null)
//
...
//
//         ImmutableArray<NamedTypeSymbol> SelectInterfaces(TypeRefSymbol[] tsignature,
//             ImmutableArray<NamedTypeSymbol> boundtypes)
//         {
...
//         }
//
//         static ImmutableArray<TraitUse> SelectTraitUses(SourceTypeSymbol owner, TypeRefSymbol[] tsignature,
//             ImmutableArray<NamedTypeSymbol> boundtypes)
//         {
//             Debug.Assert(tsignature.Length == boundtypes.Length);
...
//                     {
//                         DiagnosticBagExtensions.Add(diagnostics, CreateLocation(t.TypeRef.Span),
//                             ErrorCode.ERR_NotYetImplemented, "Using generic types");
//                     }
//
//                     if (t.Symbol is ErrorTypeSymbol err && err.CandidateReason != CandidateReason.Ambiguous)
//                     {
//                         DiagnosticBagExtensions.Add(diagnostics, CreateLocation(t.TypeRef.Span),
//                             ErrorCode.ERR_TypeNameCannotBeResolved, t.TypeRef.ClassName.ToString());
//                     }
//                 }
//             }
//
...
//                         var vinst = NewSelf();
//                         _nextVersion = InitNewSelf(vinst, v_base, v_interfaces, SelectTraitUses(vinst, tsignature, v),
...
//                         "expecting some errors if we did not resolve anything");
...
//
//                     diagnostics.Add(CreateLocation(_syntax.HeadingSpan), ErrorCode.WRN_AmbiguousDeclaration,
//                         this.FullName);
//                 }
//             }
//
...
//                 _lazyTraitUses = ImmutableArray<TraitUse>.Empty;
//                 _lazyInterfacesType = tsignature.Where(x => x.Attributes.IsInterface()).Select(x => x.Symbol)
//                     .AsImmutable();
//             }
//             else
//             {
...
//         }
//
//         bool CheckForErrors(List<Diagnostic> errors, TypeRefSymbol[] tsignature,
//             ImmutableArray<NamedTypeSymbol> boundtypes)
//         {
...
//                     errors.Add(MessageProvider.Instance.CreateDiagnostic(
//                         target.IsInterface()
//                             ? ErrorCode.ERR_CannotImplementNonInterface
//                             : ErrorCode.ERR_CannotUseNonTrait,
...
//                     if (set.Add(b) == false)
//                     {
//                         DiagnosticBagExtensions.Add(diagnostics, CreateLocation(_syntax.HeadingSpan),
//                             ErrorCode.ERR_CircularBase, t.BaseType, t);
//                         break;
//                     }
//                 }
//             }
...
//         }
//
//         static IEnumerable<ImmutableArray<T>> Variations<T>(ImmutableArray<T> types, SourceFileSymbol containingFile)
...
//             var best = ambiguity.FirstOrDefault(x =>
//                 x is SourceTypeSymbol srct && ReferenceEquals(srct.ContainingFile, containingFile) &&
//                 !srct.Syntax.IsConditional);
//             if (best != null)
//             {
...
//                     Symbol = (NamedTypeSymbol) compilation.GlobalSemantics.ResolveType(baseTypeName, resolved) ??
//                              CreateInCycleErrorType(baseTypeName),
//                 };
//             }
//             else if ((syntax.MemberAttributes & (PhpMemberAttributes.Static | PhpMemberAttributes.Interface)) != 0
//             ) // a static class or an interface
//             {
...
//                         TypeRef = namedt,
//                         Symbol =
//                             (NamedTypeSymbol) compilation.GlobalSemantics.ResolveType(namedt.ClassName, resolved) ??
//                             CreateInCycleErrorType(namedt.ClassName),
...
//                 DiagnosticBagExtensions.Add(diagnostic, CreateLocation(_syntax.HeadingSpan),
//                     ErrorCode.ERR_FinalAbstractClassDeclared);
//             }
//
...
//                     diagnostic.Add(f.Locations[0], ErrorCode.ERR_PropertyAccessibilityError, FullNameString, f.Name,
//                         basedef.DeclaredAccessibility.ToString().ToLowerInvariant(),
//                         ((IPhpTypeSymbol) basedef.ContainingType).FullName);
//                 }
//             }
//
//             // trait use check
...
//                     {
//                         DiagnosticBagExtensions.Add(diagnostic, CreateLocation(t.Span),
//                             ErrorCode.ERR_PrimitiveTypeNameMisused, t);
//                     }
//                 }
//             }
//
...
//                 {
//                     DiagnosticBagExtensions.Add(diagnostic, CreateLocation(_syntax.Name.Span),
//                         ErrorCode.WRN_TypeNameInUse, this.GetTypeKindKeyword(), FullNameString);
//                 }
//             }
//
...
//         {
//             var binder = new SemanticsBinder(DeclaringCompilation, ContainingFile.SyntaxTree, locals: null,
//                 routine: null, self: this);
//
...
//                             ? binder.BindWholeExpression(f.Initializer, BoundAccess.Read).SingleBoundElement()
//                             : null,
...
//         }
//
//         protected virtual ImmutableArray<MethodSymbol> CreateInstanceConstructors() =>
//             SynthesizedPhpCtorSymbol.CreateCtors(this);
//
//         /// <summary>
...
//                 {
//                     // ?order
//                     name += "?" + decls.TakeWhile(f => f.Syntax != this.Syntax).Count()
//                         .ToString(); // index within types with the same name
//                 }
...
//         {
//             get { return IsInterface ? TypeKind.Interface : TypeKind.Class; }
//         }
//
...
//                                 }
//
//                                 if (this.Syntax.Members.OfType<FieldDeclList>().SelectMany(list => list.Fields)
//                                     .Select(fdecl => fdecl.Name.Value).Contains(fld.Name))
//                                 {
...
//                 (FullName.Name.Value + ".php").Equals(ContainingFile.FileName,
//                     StringComparison.InvariantCultureIgnoreCase) // "file name" must match "class name .php"
//             )
//             {
...
//                         var expectedpath = PhpFileUtilities.NormalizeSlashes(Path.Combine(prefix_path.path,
//                             fullname.Substring(nsprefix.Length) + ".php"));
