...
namespace Aquila.CodeAnalysis.Symbols.Source
{
    ...
    internal partial class SourceTypeSymbol : NamedTypeSymbol, IPhpTypeSymbol
    {
        ...
        protected sealed class TraitUse
        {
            ...
            IEnumerable<Symbol> CreateMembers()
            {
                ...
                foreach (var p in Symbol.EnumerateProperties())
                {
                    ...
                    yield return new Synthesized.SynthesizedTraitFieldSymbol(ContainingType, (FieldSymbol)TraitInstanceField, p);
                }
        ...
        Synthesized.SynthesizedStaticFieldsHolder/*!*/_staticsContainer; // class __statics { ... }
        ...
        void ResolveBaseTypesNoLock(TypeRefSymbol[] tsignature, DiagnosticBag diagnostics)
        {
            ...
            foreach (var t in tsignature)
            {
                if (t.Symbol != null)
                {
                    ...
                    {
                        DiagnosticBagExtensions.Add(diagnostics, CreateLocation(t.TypeRef.Span), ErrorCode.ERR_NotYetImplemented, "Using generic types");
                    }
                    if (t.Symbol is ErrorTypeSymbol err && err.CandidateReason != CandidateReason.Ambiguous)
                    {
                        DiagnosticBagExtensions.Add(diagnostics, CreateLocation(t.TypeRef.Span), ErrorCode.ERR_TypeNameCannotBeResolved, t.TypeRef.ClassName.ToString());
                    }
                }
            }
        ...
        void CheckForCircularBase(SourceTypeSymbol t, DiagnosticBag diagnostics)
        {
            if (t.BaseType != null && t.BaseType.SpecialType != SpecialType.System_Object)
            {
                ...
                for (var b = t; b != null; b = b.BaseType as SourceTypeSymbol)
                {
                    if (set.Add(b) == false)
                    {
                        DiagnosticBagExtensions.Add(diagnostics, CreateLocation(_syntax.HeadingSpan), ErrorCode.ERR_CircularBase, t.BaseType, t);
                        break;
                    }
                }
            }
        ...
        internal virtual void GetDiagnostics(DiagnosticBag diagnostic)
        {
            if (IsAbstract && IsSealed)
            {
                ...
                DiagnosticBagExtensions.Add(diagnostic, CreateLocation(_syntax.HeadingSpan), ErrorCode.ERR_FinalAbstractClassDeclared);
            }
            ...
            foreach (var m in Syntax.Members.OfType<TraitsUse>())
            {
                foreach (var t in m.TraitsList)
                {
                    ...
                    {
                        DiagnosticBagExtensions.Add(diagnostic, CreateLocation(t.Span), ErrorCode.ERR_PrimitiveTypeNameMisused, t);
                    }
                }
            }
            ...
            if (!IsAnonymousType)
            {
                ...
                {
                    DiagnosticBagExtensions.Add(diagnostic, CreateLocation(_syntax.Name.Span), ErrorCode.WRN_TypeNameInUse, this.GetTypeKindKeyword(), FullNameString);
                }
            }
