 ...
 namespace Aquila.CodeAnalysis.Symbols
{
    ...
    internal abstract partial class SourceRoutineSymbol : MethodSymbol
    {
        ...
        // TODO : WHAT is Signature in this context?
        ...
        public virtual void GetDiagnostics(DiagnosticBag diagnostic)
        {
            ...
            for (int i = 0; i < ps.Length; i++)
            {
                ...
                if (p.IsVariadic && i < ps.Length - 1)
                {
                    ...
                    diagnostic.Add(this, p, ErrorCode.ERR_VariadicParameterNotLast);
                }
                ...
                if (p.IsConstructorProperty)
                {
                        ...
                        !(this is SourceMethodSymbol))
                    {
                        ...
                        diagnostic.Add(this, p, ErrorCode.ERR_CtorPropertyNotCtor);
                    }
                    else if (this.IsAbstract || this.ContainingType.IsInterface)
                    {
                        diagnostic.Add(this, p, ErrorCode.ERR_CtorPropertyAbstractCtor);
                    }
                    else if (this.IsStatic) // function, static method, static ctor
                    {
                        diagnostic.Add(this, p, ErrorCode.ERR_CtorPropertyStaticCtor);
                    }
                    ...
                    {
                        diagnostic.Add(this, p, ErrorCode.ERR_PropertyTypeNotAllowed, this.ContainingType.PhpName(), p.Name, pt.PrimitiveTypeName.ToString());
                    }
                    else if (p.TypeHint is ReservedTypeRef rt && (
                        rt.Type == ReservedTypeRef.ReservedType.@static))
                    {
                        diagnostic.Add(this, p, ErrorCode.ERR_PropertyTypeNotAllowed, this.ContainingType.PhpName(), p.Name, rt.Type.ToString());
                    }
                    else if (p.IsVariadic)
                    {
                        diagnostic.Add(this, p, ErrorCode.ERR_CtorPropertyVariadic);
                    }
                }
            }
        }
        ...
        public override string GetDocumentationCommentXml(CultureInfo preferredCulture = null, bool expandIncludes = false, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (PHPDocBlock != null)
            {
                ...
                {
                    DocumentationCommentCompiler.WriteRoutine(output, this);
