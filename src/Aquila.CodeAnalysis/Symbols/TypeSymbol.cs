...
{
    /// <summary>
    /// A TypeSymbol is a base class for all the symbols that represent a type in Aquila.
    /// </summary>
    internal abstract partial class TypeSymbol : NamespaceOrTypeSymbol, ITypeSymbol, ITypeSymbolInternal
    {
        ...
        bool ITypeSymbol.IsNativeIntegerType =>
            SpecialType == SpecialType.System_IntPtr || SpecialType == SpecialType.System_UIntPtr;
        ...
        public MethodSymbol[] LookupMethods(string name)
        {
            ...
            if (set.Count == 0 ||
                (this.IsAbstract &&
                 set.All(m =>
                     m.IsAbstract))) // abstract or interface, otherwise all methods should be declared on this already
            {
