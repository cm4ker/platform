...
namespace Aquila.CodeAnalysis.Symbols
{
    ...
    {
        private Compilation _declaringCompilation;
        ...
        Compilation ISymbolInternal.DeclaringCompilation => _declaringCompilation;
        ...
        public virtual ImmutableArray<NamedTypeSymbol> AllInterfaces
        {
            get
            {
                ...
                void CollectTypes(NamedTypeSymbol type, HashSet<NamedTypeSymbol> resolved, ref int interfaces,
                    ref ImmutableArray<NamedTypeSymbol> onlyinterfaces)
                {
                    if (type != null && resolved.Add(type))
                    {
                        ...
                        if (ifaces.IsDefaultOrEmpty == false)
                        {
                            ...
                            onlyinterfaces = onlyinterfaces.IsDefault
                                ? ifaces
                                : ImmutableArray<NamedTypeSymbol>
                                    .Empty; // only if we reach a single array of interfaces in whole hierarchy
        ...
        {
            return this.IsTypeParameter()
                ? ((TypeParameterSymbol) this).EffectiveBaseClass(ref useSiteDiagnostics)
                : this;
        }
        ...
        /// </summary>
        internal bool IsEqualToOrDerivedFrom(TypeSymbol type, bool ignoreDynamic,
            ref HashSet<DiagnosticInfo> useSiteDiagnostics)
        {
            return this.Equals(type, ignoreDynamic: ignoreDynamic) ||
        ...
        internal virtual bool Equals(TypeSymbol t2, bool ignoreCustomModifiersAndArraySizesAndLowerBounds = false,
        ...
        public MethodSymbol[] LookupMethods(string name)
        {
            ...
            TypeSymbol
                topPhpType =
                    null; // deals with PHP-like overriding, once there is PHP method that override another method (even with a different signature) in a base PHP type
            ...
            if (set.Count == 0 || (this.IsAbstract && set.All(m => m.IsAbstract))
            ) // abstract or interface, otherwise all methods should be declared on this already
            {
