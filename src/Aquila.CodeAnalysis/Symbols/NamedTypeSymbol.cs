 ...
 namespace Aquila.CodeAnalysis.Symbols
{
    ...
    internal abstract partial class NamedTypeSymbol : TypeSymbol, INamedTypeSymbolInternal
    {
        ...
        internal NamedTypeSymbol(Tuples.NamedTypeSymbol.TupleExtraData tupleData = null)
        {
        ...
        internal bool IsTupleTypeOfCardinality(out int tupleCardinality)
        {
            ...
            if (!IsUnboundGenericType &&
                ...
                Name == Tuples.NamedTypeSymbol.ValueTupleTypeName &&
                ContainingNamespace.Name == MetadataHelpers.SystemString)
            {
                int arity = Arity;

                if (arity >= 0 && arity < Tuples.NamedTypeSymbol.ValueTupleRestPosition)
                {
                    tupleCardinality = arity;
                    return true;
                }
                else if (arity == Tuples.NamedTypeSymbol.ValueTupleRestPosition && !IsDefinition)
                {
                    ...
                    {
                        levelsOfNesting++;
                        typeToCheck = ((NamedTypeSymbol)typeToCheck).TypeArgumentsWithAnnotationsNoUseSiteDiagnostics[Tuples.NamedTypeSymbol.ValueTupleRestPosition - 1].Type;
                    }
                    ...
                    if (arity > 0 && arity < Tuples.NamedTypeSymbol.ValueTupleRestPosition && ((NamedTypeSymbol)typeToCheck).IsTupleTypeOfCardinality(out tupleCardinality))
                    {
                        Debug.Assert(tupleCardinality < Tuples.NamedTypeSymbol.ValueTupleRestPosition);
                        tupleCardinality += (Tuples.NamedTypeSymbol.ValueTupleRestPosition - 1) * levelsOfNesting;
                        return true;
                    }
                }
            }
