...
namespace Aquila.CodeAnalysis.Symbols
{
    ...
    internal abstract partial class NamedTypeSymbol : TypeSymbol, INamedTypeSymbol, INamedTypeSymbolInternal
    {
        ...
        {
            get { return MangleName ? MetadataHelpers.ComposeAritySuffixedMetadataName(Name, Arity) : Name; }
        }
            ...
            bool ignoreDynamic = false)
        {
            ...
            if (((object)this == (object)thisOriginalDefinition ||
                ...
                !(ignoreCustomModifiersAndArraySizesAndLowerBounds &&
                  (this.HasTypeArgumentsCustomModifiers || other.HasTypeArgumentsCustomModifiers)))
            {
                return false;
            }
        ...
        /// </summary>
        private bool EqualsComplicatedCases(NamedTypeSymbol other,
            bool ignoreCustomModifiersAndArraySizesAndLowerBounds = false, bool ignoreDynamic = false)
        {
            if ((object)this.ContainingType != null &&
                !this.ContainingType.Equals(other.ContainingType, ignoreCustomModifiersAndArraySizesAndLowerBounds,
                    ignoreDynamic))
            {
                return false;
            }
            ...
            if (!ignoreCustomModifiersAndArraySizesAndLowerBounds &&
                hasTypeArgumentsCustomModifiers != other.HasTypeArgumentsCustomModifiers)
            {
                return false;
            }

            var typeArguments = ImmutableArrayExtensions.ToArray<TypeSymbol>(this.TypeArgumentsNoUseSiteDiagnostics);
            var otherTypeArguments =
                ImmutableArrayExtensions.ToArray<TypeSymbol>(other.TypeArgumentsNoUseSiteDiagnostics);
