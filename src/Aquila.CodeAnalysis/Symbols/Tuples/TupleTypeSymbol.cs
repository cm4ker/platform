...
#nullable enable
 ...
 namespace Aquila.CodeAnalysis.Symbols.Tuples
{
    internal abstract partial class NamedTypeSymbol
    {
        ...
        /// </summary>
        internal static Symbols.NamedTypeSymbol CreateTuple(
            ...
            DiagnosticBag? diagnostics = null)
        {
            ...
            Symbols.NamedTypeSymbol underlyingType = getTupleUnderlyingType(elementTypesWithAnnotations, syntax, compilation, diagnostics);
            ...
            if (diagnostics != null && ((SourceModuleSymbol)compilation.SourceModule).AnyReferencedAssembliesAreLinked)
            {
                ...
                Microsoft.CodeAnalysis.Emit.NoPia.EmbeddedTypesManager.IsValidEmbeddableType(underlyingType, syntax, diagnostics);
            }
            ...
            static Symbols.NamedTypeSymbol getTupleUnderlyingType(ImmutableArray<TypeWithAnnotations> elementTypes, CSharpSyntaxNode? syntax, CSharpCompilation compilation, DiagnosticBag? diagnostics)
            {
                int numElements = elementTypes.Length;
                int remainder;
                int chainLength = NumberOfValueTuples(numElements, out remainder);

                Symbols.NamedTypeSymbol firstTupleType = compilation.GetWellKnownType(GetTupleType(remainder));
                if (diagnostics is object && syntax is object)
                {
                    ReportUseSiteAndObsoleteDiagnostics(syntax, diagnostics, firstTupleType);
                }

                Symbols.NamedTypeSymbol? chainedTupleType = null;
                if (chainLength > 1)
                {
        ...
        public static Symbols.NamedTypeSymbol CreateTuple(
            Symbols.NamedTypeSymbol tupleCompatibleType,
        ...
        internal Symbols.NamedTypeSymbol WithTupleDataFrom(Symbols.NamedTypeSymbol original)
        {
        ...
        internal Symbols.NamedTypeSymbol? TupleUnderlyingType
        ...
        /// </summary>
        internal Symbols.NamedTypeSymbol WithElementTypes(ImmutableArray<TypeWithAnnotations> newElementTypes)
        {
            Debug.Assert(TupleElementTypesWithAnnotations.Length == newElementTypes.Length);
            Debug.Assert(newElementTypes.All(t => t.HasType));

            Symbols.NamedTypeSymbol firstTupleType;
            Symbols.NamedTypeSymbol? chainedTupleType;
            ...
            else
            {
                ...
                do
                {
                    underlyingType = ((Symbols.NamedTypeSymbol)underlyingType.TypeArgumentsWithAnnotationsNoUseSiteDiagnostics[ValueTupleRestIndex].Type);
                }
                while (underlyingType.Arity >= ValueTupleRestPosition);
        ...
        /// </summary>
        internal Symbols.NamedTypeSymbol WithElementNames(ImmutableArray<string?> newElementNames,
        ...
        private Symbols.NamedTypeSymbol WithTupleData(TupleExtraData newData)
        {
            Debug.Assert(IsTupleType);
        ...
        protected abstract Symbols.NamedTypeSymbol WithTupleDataCore(TupleExtraData newData);
        ...
        /// </summary>
        internal static void GetUnderlyingTypeChain(Symbols.NamedTypeSymbol underlyingTupleType, ArrayBuilder<Symbols.NamedTypeSymbol> underlyingTupleTypeChain)
        {
            Symbols.NamedTypeSymbol currentType = underlyingTupleType;
            ...
            while (true)
            {
                ...
                {
                    currentType = (Symbols.NamedTypeSymbol)currentType.TypeArgumentsWithAnnotationsNoUseSiteDiagnostics[ValueTupleRestPosition - 1].Type;
                }
                else
                {
                    break;
        ...
        private static Symbols.NamedTypeSymbol ConstructTupleUnderlyingType(Symbols.NamedTypeSymbol firstTupleType, Symbols.NamedTypeSymbol? chainedTupleTypeOpt, ImmutableArray<TypeWithAnnotations> elementTypes)
        {
            ...
            Symbols.NamedTypeSymbol currentSymbol = firstTupleType.Construct(ImmutableArray.Create(elementTypes, (chainLength - 1) * (ValueTupleRestPosition - 1), remainder));
        ...
        private static void ReportUseSiteAndObsoleteDiagnostics(CSharpSyntaxNode? syntax, DiagnosticBag diagnostics, Symbols.NamedTypeSymbol firstTupleType)
        {
        ...
        internal static void VerifyTupleTypePresent(int cardinality, CSharpSyntaxNode? syntax, CSharpCompilation compilation, DiagnosticBag diagnostics)
        {
            ...
            if (chainLength > 1)
            {
                Symbols.NamedTypeSymbol chainedTupleType = compilation.GetWellKnownType(GetTupleType(ValueTupleRestPosition));
                ReportUseSiteAndObsoleteDiagnostics(syntax, diagnostics, chainedTupleType);
            }
        }
        ...
        /// </summary>
        internal static Symbol? GetWellKnownMemberInType(Symbols.NamedTypeSymbol type, WellKnownMember relativeMember, DiagnosticBag diagnostics, SyntaxNode? syntax)
        {
            ...
            static Symbol? GetWellKnownMemberInType(Symbols.NamedTypeSymbol type, WellKnownMember relativeMember)
            {
        ...
        protected ArrayBuilder<Symbol> AddOrWrapTupleMembers(ImmutableArray<Symbol> currentMembers)
        {
            ...
            Symbols.NamedTypeSymbol currentValueTuple = this;
            ...
            while (true)
            {
                ...
                currentValueTuple = (Symbols.NamedTypeSymbol)oldUnderlying.TypeArgumentsWithAnnotationsNoUseSiteDiagnostics[ValueTupleRestIndex].Type;
                currentNestingLevel++;
            ...
            for (int i = 0; i < elementsMatchedByFields.Count; i++)
            {
                if (!elementsMatchedByFields[i])
                {
                    ...
                    Symbols.NamedTypeSymbol container = getNestedTupleUnderlyingType(this, fieldChainLength - 1).OriginalDefinition;
            ...
            static Symbols.NamedTypeSymbol getNestedTupleUnderlyingType(Symbols.NamedTypeSymbol topLevelUnderlyingType, int depth)
            {
                Symbols.NamedTypeSymbol found = topLevelUnderlyingType;
                for (int i = 0; i < depth; i++)
                {
                    found = (Symbols.NamedTypeSymbol)found.TypeArgumentsWithAnnotationsNoUseSiteDiagnostics[ValueTupleRestPosition - 1].Type;
                }
        ...
        private TypeSymbol MergeTupleNames(Symbols.NamedTypeSymbol other, Symbols.NamedTypeSymbol mergedType)
        {
        ...
        internal sealed class TupleExtraData
        {
            ...
            /// </summary>
            internal Symbols.NamedTypeSymbol TupleUnderlyingType { get; }

            internal TupleExtraData(Symbols.NamedTypeSymbol underlyingType)
            {
            ...
            internal TupleExtraData(Symbols.NamedTypeSymbol underlyingType, ImmutableArray<string?> elementNames,
            ...
            public ImmutableArray<TypeWithAnnotations> TupleElementTypesWithAnnotations(Symbols.NamedTypeSymbol tuple)
            {
                ...
                static ImmutableArray<TypeWithAnnotations> collectTupleElementTypesWithAnnotations(Symbols.NamedTypeSymbol tuple)
                {
            ...
            public ImmutableArray<FieldSymbol> TupleElements(Symbols.NamedTypeSymbol tuple)
            {
                Debug.Assert(tuple.Equals(TupleUnderlyingType, TypeCompareKind.IgnoreTupleNames));
                ...
                ImmutableArray<FieldSymbol> collectTupleElementFields(Symbols.NamedTypeSymbol tuple)
                {
