...
namespace Aquila.CodeAnalysis.Symbols.PE
{
    internal sealed partial class PEModuleSymbol : ModuleSymbol
    {
        ...
        /// </summary>
        readonly PEAssemblySymbol _assemblySymbol;
        ...
        internal readonly ConcurrentDictionary<TypeDefinitionHandle, TypeSymbol> TypeHandleToTypeMap =
            new ConcurrentDictionary<TypeDefinitionHandle, TypeSymbol>(concurrencyLevel: 2,
        ...
        internal readonly ConcurrentDictionary<TypeReferenceHandle, TypeSymbol> TypeRefHandleToTypeMap =
            new ConcurrentDictionary<TypeReferenceHandle, TypeSymbol>(concurrencyLevel: 2,
                capacity: DefaultTypeMapCapacity);

        public PEModuleSymbol(PEAssemblySymbol assembly, PEModule module, MetadataImportOptions importOptions,
            int ordinal)
        {
            _assemblySymbol = assembly;
            _module = module;
            _ordinal = ordinal;
                ...
                if (!assemblyMSCorLib.IsNil)
                {
                    foreach (var qualifier in Cci.MetadataWriter.dummyAssemblyAttributeParentQualifier)
                    {
                        ...
                        if (!typerefAssemblyAttributesGoHere.IsNil)
                        {
                            try
                            {
                                foreach (var customAttributeHandle in Module.GetCustomAttributesOrThrow(
        ...
        internal ImmutableArray<AttributeData> GetCustomAttributesFilterExtensions(EntityHandle token,
            out bool foundExtension)
        {
        ...
        internal override ICollection<string> TypeNames
        {
            get
            {
                ...
                {
                    Interlocked.CompareExchange(ref _lazyTypeNames, _module.TypeNames.AsCaseSensitiveCollection(),
                        null);
                }
            ...
            get
            {
                if (_lazyNamespaceNames == null)
                {
                    Interlocked.CompareExchange(ref _lazyNamespaceNames,
                        _module.NamespaceNames.AsCaseSensitiveCollection(), null);
                }
        ...
        private NamedTypeSymbol GetTypeSymbolForWellKnownType(WellKnownType type)
        {
            ...
            {
                NamedTypeSymbol currResult =
                    assembly.LookupTopLevelMetadataType(ref emittedName, digThroughForwardedTypes: true);
        ...
        internal AssemblySymbol GetAssemblyForForwardedType(ref MetadataTypeName fullName)
        {
            ...
            {
                string matchedName;
                (int firstIndex, int secondIndex) = Module.GetAssemblyRefsForForwardedType(fullName.FullName,
                    ignoreCase: false, matchedName: out matchedName);
