...
namespace Aquila.CodeAnalysis.Symbols.PE
{
    ...
    internal class PEParameterSymbol : ParameterSymbol
    {
        ...
        private struct PackedFlags
        {
            ...
            private const int AllWellKnownAttributesCompleteNoData =
                WellKnownAttributeCompletionFlagMask << WellKnownAttributeCompletionFlagOffset;
        ...
        internal static PEParameterSymbol Create(
            ...
            out bool isBad)
        {
            return Create(moduleSymbol, containingSymbol, ordinal, parameter.IsByRef, parameter.Type, parameter.Handle,
                parameter.CustomModifiers, out isBad);
        }
        ...
        internal static PEParameterSymbol Create(
            ...
            out bool isBad)
        {
            return Create(moduleSymbol, containingSymbol, ordinal, parameter.IsByRef, parameter.Type, handle,
                parameter.CustomModifiers, out isBad);
        }
        ...
        {
            get { return _packedFlags.HasNameInMetadata; }
        }
            ...
            out bool isBad)
        {
            ...
            return new PEParameterSymbolWithCustomModifiers(moduleSymbol, containingSymbol, ordinal, isByRef, type,
                handle, customModifiers, out isBad);
        }
        ...
        {
            get { return _packedFlags.RefKind; }
        }
        ...
        {
            get { return HasNameInMetadata ? _name : string.Empty; }
        }
        ...
        {
            get { return (_flags & ParameterAttributes.HasDefault) != 0; }
        }
            ...
            get
            {
                ...
                if (!_packedFlags.TryGetWellKnownAttribute(flag, out var value))
                {
                    value = _packedFlags.SetWellKnownAttribute(flag,
                        AttributeHelpers.HasNotNullAttribute(Handle, (PEModuleSymbol)ContainingModule));
                }
                ...
                if (!_packedFlags.HasDefaultValueFieldPopulated)
                {
                    if (AttributeHelpers.HasDefaultValueAttributeData(Handle, (PEModuleSymbol)ContainingModule))
                    {
                        var attr = GetAttributes()
                            .FirstOrDefault(attr => attr.AttributeClass.Name == "DefaultValueAttribute");
                        if (attr != null)
                        {
                            // [DefaultValueAttribute( FieldName ) { ExplicitType }]
                            var fldname = (string)attr.ConstructorArguments[0].Value;
                            var container =
                                attr.NamedArguments.SingleOrDefault(pair => pair.Key == "ExplicitType").Value
