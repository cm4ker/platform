...
namespace Aquila.CodeAnalysis.Symbols.PE
{
    ...
    internal abstract class PENamedTypeSymbol : NamedTypeSymbol
    {
        ...
        private sealed class PENamedTypeSymbolGeneric : PENamedTypeSymbol
        {
            ...
            private void EnsureTypeParametersAreLoaded()
            {
                if (_lazyTypeParameters.IsDefault)
                {
                    ...
                    {
                        ownedParams[i] = new PETypeParameterSymbol(moduleSymbol, this, (ushort)i,
        ...
        internal static PENamedTypeSymbol Create(PEModuleSymbol moduleSymbol, PENamespaceSymbol containingNamespace,
            TypeDefinitionHandle handle, string emittedNamespaceName)
        {
            GenericParameterHandleCollection genericParameterHandles;
            ushort arity;
            ...
            if (arity == 0)
            {
                result = new PENamedTypeSymbolNonGeneric(moduleSymbol, containingNamespace, handle,
                    emittedNamespaceName, out mangleName);
            }
            else
            {
        ...
        private static void GetGenericInfo(PEModuleSymbol moduleSymbol, TypeDefinitionHandle handle,
            out GenericParameterHandleCollection genericParameterHandles, out ushort arity,
        ...
        private static readonly Dictionary<string, ImmutableArray<PENamedTypeSymbol>> s_emptyNestedTypes =
            new Dictionary<string, ImmutableArray<PENamedTypeSymbol>>();
            ...
            out bool mangleName)
        {
            ...
            {
                _corTypeId =
                    SpecialTypes.GetTypeFromMetadataName(
        ...
        internal override ImmutableArray<NamedTypeSymbol> GetDeclaredInterfaces(ConsList<Symbol> basesBeingResolved)
        {
            ...
            {
                ImmutableInterlocked.InterlockedCompareExchange(ref _lazyDeclaredInterfaces, MakeDeclaredInterfaces(),
            ...
            try
            {
                ...
                if (interfaceImpls.Count > 0)
                {
                    ...
                    foreach (var interfaceImpl in interfaceImpls)
                    {
                        ...
                        symbols[i++] = (object)namedTypeSymbol != null
                            ? namedTypeSymbol
                            : new UnsupportedMetadataTypeSymbol(); // interface tmpList contains a bad type
                    }
        ...
        private void LoadMembers()
        {
            ...
            if (_lazyMembersInDeclarationOrder.IsDefault)
            {
                ...
                if (this.TypeKind == TypeKind.Enum)
                {
                    ...
                    try
                    {
                        foreach (var fieldDef in module.GetFieldsOfTypeOrThrow(_handle))
                        {
                            ...
                            if (Microsoft.CodeAnalysis.ModuleExtensions.ShouldImportField(fieldFlags,
                ...
                if (!ImmutableInterlocked.InterlockedInitialize(ref _lazyMembersInDeclarationOrder,
        ...
        private void CreateProperties(Dictionary<MethodDefinitionHandle, PEMethodSymbol> methodHandleToSymbol,
        ...
        private PEMethodSymbol GetAccessorMethod(PEModule module,
            Dictionary<MethodDefinitionHandle, PEMethodSymbol> methodHandleToSymbol, MethodDefinitionHandle methodDef)
        {
            if (methodDef.IsNil)
            {
                return null;
        ...
        internal NamedTypeSymbol GetDeclaredBaseType(ConsList<Symbol> basesBeingResolved)
        {
            ...
            {
                Interlocked.CompareExchange(ref _lazyDeclaredBaseType, MakeDeclaredBaseType(),
        ...
        {
            return PEDocumentationCommentUtils.GetDocumentationComment(this, ContainingPEModule, preferredCulture,
                cancellationToken, ref _lazyDocComment);
        }
    }
}