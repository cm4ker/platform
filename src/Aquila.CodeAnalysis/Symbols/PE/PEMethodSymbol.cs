...
namespace Aquila.CodeAnalysis.Symbols.PE
{
    ...
    internal sealed class PEMethodSymbol : MethodSymbol
    {
        ...
        private class SignatureData
        {
            ...
            public SignatureData(SignatureHeader header, ImmutableArray<ParameterSymbol> parameters,
            ...
            MethodDefinitionHandle methodDef)
        {
            ...
            try
            {
                ...
                moduleSymbol.Module.GetMethodDefPropsOrThrow(methodDef, out _name, out implFlags, out localflags,
                    out rva);
        ...
        public override ImmutableArray<AttributeData> GetAttributes()
        {
            if (!_packedFlags.IsCustomAttributesPopulated)
            {
                ...
                {
                    attributeData = InterlockedOperations.Initialize(ref AccessUncommonFields()._lazyCustomAttributes,
                        attributeData);
                }
        ...
        public override bool CastToFalse
        {
            get
            {
                if (!_packedFlags.IsCastToFalseIsPopulated)
                {
                    ...
                    _packedFlags.InitializeIsCastToFalse(AttributeHelpers.HasCastToFalse(returnparam.Handle,
                        (PEModuleSymbol)returnparam.ContainingModule));
                }
        ...
        public override ImmutableArray<MethodSymbol> ExplicitInterfaceImplementations
        {
            get
            {
                ...
                var explicitlyOverriddenMethods =
                    new MetadataDecoder(moduleSymbol, _containingType).GetExplicitlyOverriddenMethods(
                        _containingType.Handle, _handle, this.ContainingType);
                ...
                _packedFlags.InitializeIsExplicitOverride(isExplicitFinalizerOverride: sawObjectFinalize,
                ...
                return InterlockedOperations.Initialize(ref _lazyExplicitMethodImplementations,
        ...
        public override bool IsSealed =>
            this.IsMetadataFinal && !this.IsAbstract && this.IsOverride; //slowest check last
        ...
        public override bool IsVirtual => this.IsMetadataVirtual() && !this.IsDestructor && !this.IsMetadataFinal &&
        ...
        private MethodKind ComputeMethodKind()
        {
            if (this.HasSpecialName)
            {
                if (_name.StartsWith(".", StringComparison.Ordinal))
                {
                    ...
                    if ((Flags & (MethodAttributes.RTSpecialName | MethodAttributes.Virtual)) ==
                        MethodAttributes.RTSpecialName &&
                        _name.Equals(this.IsStatic
            ...
            if (!this.IsStatic)
            {
                switch (_name)
                {
                    case WellKnownMemberNames.DestructorName:
                        if ((this.ContainingType.TypeKind ==
                             TypeKind.Class) || // && this.IsRuntimeFinalizer(skipFirstMethodKindCheck: true)) ||
        ...
        private bool IsValidUserDefinedOperatorSignature(int parameterCount) =>
            ...
            this.Parameters.All(p =>
                p.RefKind == RefKind.None) && //this.ParameterRefKinds.IsDefault && // No 'ref' or 'out'
            !this.IsParams();
        ...
        private SignatureData LoadSignature()
        {
            ...
            ParamInfo<TypeSymbol>[] paramInfo =
                new MetadataDecoder(moduleSymbol, this).GetSignatureForMethod(_handle, out signatureHeader, out mrEx);
            ...
            get
            {
                if (!_packedFlags.IsObsoleteAttributePopulated)
                {
                    var result = ObsoleteAttributeHelpers.GetObsoleteDataFromMetadata(_handle,
                        (PEModuleSymbol)ContainingModule, ignoreByRefLikeMarker: false);
                    if (result != null)
                    {
                        result = InterlockedOperations.Initialize(ref AccessUncommonFields()._lazyObsoleteAttributeData,
                            result, ObsoleteAttributeData.Uninitialized);
                    }
                ...
                else
                {
                    ...
                    return ReferenceEquals(result, ObsoleteAttributeData.Uninitialized)
                        ? InterlockedOperations.Initialize(ref uncommonFields._lazyObsoleteAttributeData, null,
        ...
        public override string GetDocumentationCommentXml(CultureInfo preferredCulture = null,
            bool expandIncludes = false, CancellationToken cancellationToken = default)
        {
            return PEDocumentationCommentUtils.GetDocumentationComment(this, _containingType.ContainingPEModule,
