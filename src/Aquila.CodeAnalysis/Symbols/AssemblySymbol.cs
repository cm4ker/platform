    ...
    internal abstract class AssemblySymbol : Symbol, IAssemblySymbol
    {
            ...
            DiagnosticBag warnings = null)
        {
            ...
            else
            {
                ...
                if (includeReferences && (type == null || type.IsErrorType()))
                {
                    ...
                    assemblies.UnionWith(Enumerable.Select<KeyValuePair<MetadataReference, IAssemblySymbol>, AssemblySymbol>(this.DeclaringCompilation.GetBoundReferenceManager().GetReferencedAssemblies(), x => (AssemblySymbol)x.Value));
        ...
        internal virtual Symbol GetDeclaredSpecialTypeMember(SpecialMember member)
        {
            if (_lazySpecialTypeMembers == null || ReferenceEquals(_lazySpecialTypeMembers[(int)member], ErrorTypeSymbol.UnknownResultType))
            {
                ...
                if (!type.IsErrorType())
                {
                    result = AquilaCompilation.GetRuntimeMember(type, ref descriptor, AquilaCompilation.SpecialMembersSignatureComparer.Instance, null);
                }
