            ...
            DiagnosticBag warnings = null)
        {
            ...
            else
            {
                ...
                if (includeReferences && (type == null || type.IsErrorType()))
                {
                    ...
                    assemblies.UnionWith(
                        Enumerable.Select<KeyValuePair<MetadataReference, IAssemblySymbolInternal>, AssemblySymbol>(
                            this.DeclaringCompilation.GetBoundReferenceManager().GetReferencedAssemblies(),
                            x => (AssemblySymbol)x.Value));
                ...
                ErrorTypeSymbol.UnknownResultType))
            {
                ...
                if (!type.IsErrorType())
                {
                    result = AquilaCompilation.GetRuntimeMember(type, ref descriptor,
                        AquilaCompilation.SpecialMembersSignatureComparer.Instance, null);
                }
