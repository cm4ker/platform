    ...
    internal static class OverrideHelper
    {
        ...
        public static MethodSymbol ResolveOverride(this MethodSymbol method)
        {
            ...
            foreach (var t in EnumerateOverridableTypes(method.ContainingType))
            {
                ...
                foreach (var m in t.GetMembers(method.Name).OfType<MethodSymbol>())
                {
                    ...
                    {
                        if (t.IsInterface &&
                            overriden.Count != 0) // method on interface that might be already implemented
                        {
        ...
        internal static bool IsExplicitInterfaceImplementation(this MethodSymbol method, MethodSymbol basemethod)
        {
            if (method.DeclaredAccessibility == Accessibility.Private &&
                method.ExplicitInterfaceImplementations.Contains(basemethod))
            {
        ...
        public static MethodSymbol ResolveMethodImplementation(this MethodSymbol method,
            ImmutableArray<Symbol> overridecandidates)
        {
        ...
        static IEnumerable<NamedTypeSymbol> EnumerateOverridableTypes(NamedTypeSymbol type)
        {
            ...
            Queue<NamedTypeSymbol>
                typesWithInterfaces = null; // remember abstract types which interfaces have to be returned as well
        ...
        {
            return !method.IsStatic && !method.IsSealed && method.DeclaredAccessibility != Accessibility.Private &&
        ...
        /// </summary>
        public static ConversionCost OverrideCost(ImmutableArray<ParameterSymbol> givenps,
            ImmutableArray<ParameterSymbol> baseps)
        {
            //
        ...
        static ConversionCost OverrideCost(MethodSymbol method, MethodSymbol basemethod)
        {
            ...
            //
            if (!string.Equals(method.MethodName, basemethod.MethodName, StringComparison.InvariantCultureIgnoreCase))
            {
        ...
        public static bool CanBeOverride(SourceMethodSymbolBase method, MethodSymbol basemethod)
        {
