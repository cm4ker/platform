...
namespace Aquila.CodeAnalysis.Symbols.Anonymous
{
    internal sealed class AnonymousTypeManager : CommonAnonymousTypeManager
    {
        ...
        {
            var type = new SynthesizedTypeSymbol((NamespaceOrTypeSymbol)Compilation.SourceModule.GlobalNamespace,
                Compilation);

            type.SetName(name);
            type.SetIsStatic(isStatic);
            type.SetAccess(accessibility);
