...
namespace Aquila.Core.Assemlies
{
    ...
    public class FileAssemblyStorage : IAssemblyStorage
    {
        ...
        {
            return Directory
                .GetFiles(_storagePath, string.Concat("*", FILE_DESCRIPTION_EXT), SearchOption.TopDirectoryOnly).Select(
                    path =>
        ...
        {
            return GetAssembly(GetAssemblies()
                .First(a => a.ConfigurationHash.Equals(configurationHash) && a.Name.Equals(name)));
        }
