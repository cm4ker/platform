...
namespace Aquila.ServerRPC
{
    class ArrayPrinter
    {
        ...
        private static string GetDataInTableFormat(string[,] arrValues)
        {
            ...
            //printing top line;
            formattedString = string.Format("{0}{1}{2}{3}", cellLeftTop, Indent(indentLength), cellRightTop,
            ...
            for (int i = 0; i < dimension1Length; i++)
            {
                ...
                {
                    string value = (isLeftAligned)
                        ? arrValues[i, j].PadRight(maxCellWidth, ' ')
                        : arrValues[i, j].PadLeft(maxCellWidth, ' ');
            ...
            formattedString += string.Format("{0}{1}{2}{3}", cellLeftBottom, Indent(indentLength), cellRightBottom,
                System.Environment.NewLine);
            return formattedString;
        }
