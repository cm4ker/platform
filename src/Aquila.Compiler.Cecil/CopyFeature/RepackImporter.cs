...
namespace Aquila.Compiler.Cecil.CopyFeature
{
    internal class RepackImporter
    {
        ...
        public RepackImporter(ILogger logger, RepackOptions options, IRepackContext context,
            Dictionary<AssemblyDefinition, int> aspOffests)
        {
            _logger = logger;
            _options = options;
            _repackContext = context;
            _aspOffsets = aspOffests;
        }
        ...
        public TypeDefinition Import(TypeDefinition type, Collection<TypeDefinition> col, bool internalize)
        {
            ...
            else
            {
                ...
                throw new InvalidOperationException(
                    ...
                    MappingHandler.GetScopeFullName(_repackContext.MappingHandler
                        .GetOrigTypeScope<IMetadataScope>(nt)));
            }
        ...
        private bool ShouldDrop<TMember>(TMember member) where TMember : ICustomAttributeProvider, IMemberDefinition
        {
            ...
            var shouldDrop = member.HasCustomAttributes
                             && member.CustomAttributes.FirstOrDefault(attr =>
                                 attr.AttributeType.Name == _options.RepackDropAttribute) != null;
            if (shouldDrop)
            {
                _logger.Log("Repack dropped " + typeof(TMember).Name + ": " + member.FullName +
                            " as it was marked with " + _options.RepackDropAttribute);
            }
        ...
        private void CloneTo(ParameterDefinition param, MethodDefinition context, Collection<ParameterDefinition> col)
        {
            ParameterDefinition pd =
                new ParameterDefinition(param.Name, param.Attributes, Import(param.ParameterType, context));
        ...
        private void CloneTo(MethodDefinition meth, TypeDefinition type, bool typeJustCreated)
        {
            ...
            MethodDefinition nm = new MethodDefinition(meth.Name, meth.Attributes,
                _repackContext.TargetAssemblyMainModule.TypeSystem.Void);
            ...
            if (meth.HasPInvokeInfo)
            {
                ...
                {
                    nm.PInvokeInfo = new PInvokeInfo(meth.PInvokeInfo.Attributes, meth.PInvokeInfo.EntryPoint,
                        meth.PInvokeInfo.Module);
                }
            }
            ...
            for (int i = 0; i < body.Instructions.Count; i++)
            {
                ...
                switch (instr.OpCode.OperandType)
                {
                    ...
                    case OperandType.InlineBrTarget:
                        ...
                        break;
                    case OperandType.InlineSwitch:
                        instr.Operand = ((Instruction[]) body.Instructions[i].Operand)
                            .Select(op => GetInstruction(body, nb, op)).ToArray();
                        break;
                    default:
                        break;
                }
        ...
        private TypeDefinition CreateType(TypeDefinition type, Collection<TypeDefinition> col, bool internalize,
            string rename)
        {
        ...
        private MethodDefinition FindMethodInNewType(TypeDefinition nt, MethodDefinition methodDefinition)
        {
            ...
            {
                _logger.Warn(
                    "Method '" + methodDefinition.FullName + "' not found in merged type '" + nt.FullName + "'");
            }

            return ret;
        }

        private void FixAspNetOffset(Collection<Instruction> instructions, MethodReference operand,
            MethodDefinition parent)
        {
            if (operand.Name == "WriteUTF8ResourceString" || operand.Name == "CreateResourceBasedLiteralControl")
            {
                ...
                if (fullName ==
                    "System.Void System.Web.UI.TemplateControl::WriteUTF8ResourceString(System.Web.UI.HtmlTextWriter,System.Int32,System.Int32,System.Boolean)" ||
                    fullName ==
                    "System.Web.UI.LiteralControl System.Web.UI.TemplateControl::CreateResourceBasedLiteralControl(System.Int32,System.Int32,System.Boolean)"
                )
                {
                    int offset;
        ...
        private bool DuplicateTypeAllowed(TypeDefinition type)
        {
            ...
            if (!String.IsNullOrEmpty(nameSpace) &&
                _options.AllowedDuplicateNameSpaces.Any(s => s == nameSpace || nameSpace.StartsWith(s + ".")))
                return true;
        ...
        /// </summary>
        public void CopySecurityDeclarations(Collection<SecurityDeclaration> input,
            Collection<SecurityDeclaration> output, IGenericParameterProvider context)
        {
...
//                }
        ...
        public void CopyGenericParameters(Collection<GenericParameter> input, Collection<GenericParameter> output,
            IGenericParameterProvider nt)
        {
            ...
            Copy(input, output, (gp, ngp) => CopyTypeConstraints(gp.Constraints, ngp.Constraints, nt));
            Copy(input, output, (gp, ngp) => CopyCustomAttributes(gp.CustomAttributes, ngp.CustomAttributes, nt));
        }

        public void CopyCustomAttributes(Collection<CustomAttribute> input, Collection<CustomAttribute> output,
        ...
        public void CopyCustomAttributes(Collection<CustomAttribute> input, Collection<CustomAttribute> output,
            bool allowMultiple, IGenericParameterProvider context)
        {
            ...
            foreach (CustomAttribute ca in input)
            {
                ...
                var similarAttributes =
                    output.Where(attr => reflectionHelper.AreSame(attr.AttributeType, caType)).ToList();
        ...
        {
            if (type.FullName == "IKVM.Attributes.JavaModuleAttribute" ||
                type.FullName == "IKVM.Attributes.PackageListAttribute")
            {
            ...
            if (typeDef != null)
            {
                var ca = typeDef.CustomAttributes.FirstOrDefault(x =>
                    x.AttributeType.FullName == "System.AttributeUsageAttribute");
                if (ca != null)
                {
        ...
        public void CopyTypeConstraints(Collection<GenericParameterConstraint> input,
            Collection<GenericParameterConstraint> output, IGenericParameterProvider provider)
        {
        }

        public TypeReference CopyType(TypeReference input, IGenericParameterProvider provider)
        {
            return Import(input, provider);
        }

        public void CopyTypeReferences(Collection<TypeReference> input, Collection<TypeReference> output,
            IGenericParameterProvider context)
        {
        ...
        {
            return new CustomAttributeArgument(Import(arg.Type, context),
                ImportCustomAttributeValue(arg.Value, context));
        }

        public CustomAttributeNamedArgument Copy(CustomAttributeNamedArgument namedArg,
            IGenericParameterProvider context)
        {
