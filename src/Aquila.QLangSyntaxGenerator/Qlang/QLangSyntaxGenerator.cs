...
{
    public static class QLangSyntaxGeneratorHelper
    {
        ...
        private static ClassDeclarationSyntax astTreeBaseCls;
        private static ClassDeclarationSyntax astTreeBaseCls2;
        ...
        public static CSharpSyntaxNode Generate(GeneratorExecutionContext context)
        {
            astTreeBaseCls = SyntaxFactory.ClassDeclaration("QLangVisitorBase<T>")
                .AddModifiers(publicToken)
                .AddModifiers(SyntaxFactory.Token(SyntaxKind.AbstractKeyword))
                .AddModifiers(partialToken);

            astTreeBaseCls2 = SyntaxFactory.ClassDeclaration("QLangVisitorBase")
                .AddModifiers(publicToken)
                .AddModifiers(SyntaxFactory.Token(SyntaxKind.AbstractKeyword))
                .AddModifiers(partialToken);


            var input = context.AdditionalFiles.SingleOrDefault(a => Path.GetFileName(a.Path) == "QLang.xml");

            if (input == null)
            {
                context.ReportDiagnostic(Diagnostic.Create(
                    new DiagnosticDescriptor("GEN001", "File QLang.xaml not found", "Test", "test",
                        DiagnosticSeverity.Error, true), null));
                return SyntaxFactory.EmptyStatement();
            }

            using (TextReader tr = new StreamReader(input.Path))
            {
        ...
        public static string GenerateRewriter(GeneratorExecutionContext context)
        {
            var input = context.AdditionalFiles.SingleOrDefault(a => Path.GetFileName(a.Path) == "QLang.xml");
...
");
                ...
                sb.AppendLine("public class QLangTreeRewriter : QLangVisitorBase<QLangElement> {");
                foreach (var syntax in root.Syntaxes)
                {
                    if (syntax.IsAbstract) continue;


                    sb.AppendLine($"public override QLangElement Visit{syntax.Name}({syntax.Name} arg) {{");

                    if (syntax.IsList)
                    {
                        sb.AppendLine(
                            $"return new {syntax.Name}(arg.Select(x => ({syntax.Base})Visit(x)).ToImmutableArray());");
                    }
                    else
                    {
                        var sb_n = new StringBuilder();
                        sb_n.Append($"return new {syntax.Name}(");
                        var needComma = false;
                        foreach (var argument in syntax.Arguments)
                        {
                            if (argument.Type.StartsWith("Q") && argument.Type != "QJoinType" &&
                                argument.Type != "QSortDirection")
                                sb.AppendLine(
                                    $"var {argument.Name.ToCamelCase()} = ({argument.Type})Visit(arg.{argument.Name});");
                            else
                                sb.AppendLine(
                                    $"var {argument.Name.ToCamelCase()} = arg.{argument.Name};");
                            ...
                            sb_n.Append(argument.Name.ToCamelCase());
                        }

                        sb_n.Append(");");

                        sb.AppendLine(sb_n.ToString());
                    }
