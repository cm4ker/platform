...
namespace Peachpie.NET.Sdk.Versioning
{
    ...
    public abstract class ComposerVersionExpression
    {
        struct Tokens
        {
            ...
            public static readonly Regex AndRegex = new Regex(@"[^|,\s]+([,\s]+)[^|,\s]+",
        ...
        public static bool TryParse(string value, out ComposerVersionExpression expression) =>
            TryParse(value.AsSpan(), out expression);
                        ...
                        UpperBound =
                            (Version.IsAnyMajor
                                    ...
                                    : Version.IsAnyMinor // * or unspecified
                                        ? (Version.PartsCount == 1
                                            ? new ComposerVersion(Version.Major, 0, 0) /* <1 */
                                            : new ComposerVersion(Version.Major + 1, 0, 0) /* <1.* */)
                                        : Version.IsAnyBuild // * or unspecified
                                            ? (Version.PartsCount == 2
                                                ? new ComposerVersion(Version.Major, Version.Minor, 0) /* <1.2 */
                                                : new ComposerVersion(Version.Major, Version.Minor + 1, 0) /* <1.2.* */)
                                            : new ComposerVersion(Version.Major, Version.Minor,
                                                Version.Build) /* <1.2.3 */
                            ).WithStabilityFlag(!Version.IsStable
                                ? ComposerVersion.StabilityStable
                                : Version.Stability ?? ComposerVersion.StabilityDev), // <dev = stable
                        UpperBoundExclusive = true,
                    };
