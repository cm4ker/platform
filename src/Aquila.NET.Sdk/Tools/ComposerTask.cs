...
namespace Aquila.NET.Sdk.Tools
{
    ...
    public class ComposerTask : Task
    {
        ...
        void LogError(ErrorCodes code, Location location, string message)
        {
            ...
            {
                Log.LogError(null, ErrorCode(code), null, path, location.Start.Line, location.Start.Col,
                    location.End.Line, location.End.Col, message);
            }
            else
            {
                Log.LogWarning(null, ErrorCode(code), null, path, location.Start.Line, location.Start.Col,
                    location.End.Line, location.End.Col, message);
            }
        }
                ...
                switch (node.Key.ToLowerInvariant())
                {
                    ...
                    case "suggest":
                        if (ComposerIncludeSuggestPackages)
                        {
                            ...
                            // { "name": "description", }
                            Dependencies = Dependencies.Concat(GetDependencies(node.Value, ignoreVersion: true))
                                .AsArray();
                        }
                            ...
                            switch (autoload.Key.ToLowerInvariant())
                            {
                                case "psr-4":
                                    Autoload_PSR4 = Autoload_PSR4
                                        .Concat(GetAutoloadPsr4FromPsr4(autoload.Value.AsObject)).AsArray();
                                    break;

                                case "psr-0":
                                    Autoload_PSR4 = Autoload_PSR4
                                        .Concat(GetAutoloadPsr4FromPsr0(autoload.Value.AsObject)).AsArray();
                                    break;

                                case "classmap":
                                    Autoload_ClassMap = Autoload_ClassMap.Concat(
                                        GetAutoloadClassMapString(autoload.Value.AsArray, false)
                                ...
                                case "exclude-from-classmap":
                                    Autoload_ClassMap_Exclude = Autoload_ClassMap_Exclude.Concat(
                                        GetAutoloadClassMapString(autoload.Value.AsArray, true)
                                            .Select(path => new TaskItem(path))).ToArray();
                                    break;
                                ...
                                default:
                                    // ???
                                    LogError(ErrorCodes.WRN_Unhandled, node.Value.Location,
                                        $"autoload \"{autoload.Key}\" key is not handled.");
                                    break;
                            }
                        }
        ...
        {
            foreach (var inf in typeof(ComposerTask).Assembly
        ...
        /// </summary>
        static readonly Dictionary<string, string> s_knowndeps_to_packageid =
        ...
        {
            return new TaskItem("PackageDependency",
                new Dictionary<string, string>(StringComparer.InvariantCultureIgnoreCase)
                {
                    {"Name", name},
                    {"Version", version},
        ...
        IEnumerable<ITaskItem> GetDependencies(JSONNode require, bool ignoreVersion = false)
        {
            ...
            foreach (var r in require)
            {
                ...
                yield return PackageDependencyItem(
                    ...
                    version: VersionRangeToPackageVersion(ignoreVersion ? string.Empty : r.Value.Value,
                        ForcePreReleaseDependency, r.Value.Location));
            }
        }
        ...
        string VersionRangeToPackageVersion(string value, bool forcePreRelease, Location location)
        {
            ...
            Peachpie.NET.Sdk.Versioning.FloatingVersion version;

            if (Peachpie.NET.Sdk.Versioning.ComposerVersionExpression.TryParse(value, out var expression))
            {
                version = expression.Evaluate();
            }
            else
            {
                LogError(ErrorCodes.WRN_InvalidDependencyVersion, location,
                    $"Version expression '{value}' is invalid.");
                version = new Peachpie.NET.Sdk.Versioning.FloatingVersion(); // any version
            }
