...
namespace Aquila.Language.Ast.Binding
{
    internal static class BoundNodeFactory
    {
        ...
        public static BoundVariableDeclaration VariableDeclaration(SyntaxNode syntax, LocalSymbol symbol, BoundExpression initializer)
        {
        ...
        {
            var local = new LocalLocalSymbol(name, isReadOnly, initializer.NamedType, initializer.ConstantValue);
        ...
        public static BoundAssignmentExpression Assignment(SyntaxNode syntax, LocalSymbol local, BoundExpression expression)
        {
            return new BoundAssignmentExpression(syntax, local, expression);
        }

        public static BoundBinaryExpression Binary(SyntaxNode syntax, BoundExpression left, SyntaxKind kind, BoundExpression right)
        {
            var op = BoundBinaryOperator.Bind(kind, left.NamedType, right.NamedType)!;
        ...
        public static BoundExpressionStatement Increment(SyntaxNode syntax, BoundVariableExpression variable)
        {
            ...
            var incrementAssign = new BoundAssignmentExpression(syntax, variable.Local, increment);
        ...
        {
            // Debug.Assert(condition.NamedType == NamedTypeSymbol.Bool);
            //
            // var op = BoundUnaryOperator.Bind(SyntaxKind.BangToken, NamedTypeSymbol.Bool);
        ...
        {
            return Variable(syntax, variable.Local);
        }

        public static BoundVariableExpression Variable(SyntaxNode syntax, LocalSymbol local)
        {
