...
// See the LICENSE file in the project root for more information.
 ...
 using System.Text;
 using Aquila.Language.Ast.PooledObjects;
 ...
 namespace Aquila.Language.Ast.Extension
{
    internal static class MetadataHelpers
    {
        ...
        private struct SerializedTypeDecoder
        {
            ...
            {
                get { return _offset >= _input.Length; }
            }
                ...
                while (!EndOfInput)
                {
                    ...
                    if (i >= 0)
                    {
                        ...
                        switch (c)
                        {
                            ...
                            default:
                                throw new Exception(); // ExceptionUtilities.UnexpectedValue(c);
                        }
                    }
                    else
                    {
            ...
            private static void HandleDecodedTypeName(string decodedTypeName, bool decodingTopLevelType,
                ref string topLevelType, ref ArrayBuilder<string> nestedTypesBuilder)
            {
                ...
                while (!EndOfInput)
                {
                    ...
                    if (!EndOfInput)
                    {
                        switch (Current)
                        {
                            ...
                            default:
                                throw new Exception( ); // ExceptionUtilities.UnexpectedValue(EndOfInput);
                        }
                    }
                }
        ...
        private static short InferTypeArityFromMetadataName(string emittedTypeName, out int suffixStartsAt)
        {
            ...
            int arity;
            bool nonNumericCharFound = !int.TryParse(stringRepresentingArity, NumberStyles.None,
                CultureInfo.InvariantCulture, out arity);
            ...
            out IEnumerable<KeyValuePair<string, IEnumerable<IGrouping<string, TypeDefinitionHandle>>>> namespaces)
        {
            ...
            var nestedNamespaces =
                new List<KeyValuePair<string, IEnumerable<IGrouping<string, TypeDefinitionHandle>>>>();
...
/*empty statement*/
                ...
                if (names.Count != nestedNamespaces.Count) // nothing to merge otherwise
                {
                    for (int i = 1; i < nestedNamespaces.Count; i++)
                    {
                        ...
                        if (keyIndex != i)
                        {
                            ...
                            throw new Exception("Look ad TODO");

                            nestedNamespaces[i] =
                                default(KeyValuePair<string, IEnumerable<IGrouping<string, TypeDefinitionHandle>>>);
                        }
                    }
        ...
        internal static bool SplitNameEqualsFullyQualifiedName(string namespaceName, string typeName,
