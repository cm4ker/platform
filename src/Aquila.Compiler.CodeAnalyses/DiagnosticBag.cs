...
namespace Aquila.Language.Ast
{
    internal sealed class DiagnosticBag : IEnumerable<Diagnostic>
    {
        ...
        public void ReportInvalidNumber(TextLocation location, string text, SpecialType specialType)
        {
            var message = $"The number {text} isn't valid {specialType.GetMetadataName()}.";
        ...
        public void ReportUndefinedUnaryOperator(TextLocation location, string operatorText,
            NamedTypeSymbol operandNamedType)
        {
            var message = $"Unary operator '{operatorText}' is not defined for type '{operandNamedType}'.";
            ReportError(location, message);
        }

        public void ReportUndefinedBinaryOperator(TextLocation location, string operatorText,
            NamedTypeSymbol leftNamedType, NamedTypeSymbol rightNamedType)
        {
            var message =
                $"Binary operator '{operatorText}' is not defined for types '{leftNamedType}' and '{rightNamedType}'.";
            ReportError(location, message);
        }
        ...
        public void ReportCannotConvert(TextLocation location, NamedTypeSymbol fromNamedType,
            NamedTypeSymbol toNamedType)
        {
            var message = $"Cannot convert type '{fromNamedType}' to '{toNamedType}'.";
            ReportError(location, message);
        }

        public void ReportCannotConvertImplicitly(TextLocation location, NamedTypeSymbol fromNamedType,
            NamedTypeSymbol toNamedType)
        {
            var message =
                $"Cannot convert type '{fromNamedType}' to '{toNamedType}'. An explicit conversion exists (are you missing a cast?)";
            ReportError(location, message);
        }
        ...
        public void ReportMissingReturnExpression(TextLocation location, NamedTypeSymbol returnNamedType)
        {
            var message = $"An expression of type '{returnNamedType}' is expected.";
            ReportError(location, message);
        }
